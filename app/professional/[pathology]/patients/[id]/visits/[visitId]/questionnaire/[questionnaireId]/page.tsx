...
import { SOCIAL_DEFINITION } from "@/lib/constants/questionnaires-social";
import {
  TOBACCO_DEFINITION,
...
import { getSocialResponse } from "@/lib/services/questionnaire-social.service";
import {
...
function shouldUseBipolarTables(
  pathology: string,
  ...
  if (
    pathology !== "bipolar" ||
    !visitType ||
    !["initial_evaluation", "annual_evaluation", "biannual_followup"].includes(
      visitType,
    )
  ) {
    return false;
  ...
  return (
    bipolarKey !== null && BIPOLAR_INITIAL_TABLES[bipolarKey] !== undefined
  );
}
...
export default async function ProfessionalQuestionnairePage({
  ...
  else if (code === DIAGNOSTIC_DEFINITION.code)
    questionnaire = DIAGNOSTIC_DEFINITION;
  else if (code === ORIENTATION_DEFINITION.code)
    questionnaire = ORIENTATION_DEFINITION;
  ...
  // Hetero questionnaires
  ...
  else if (code === ETAT_PATIENT_DEFINITION.code)
    questionnaire = ETAT_PATIENT_DEFINITION;
  ...
  else if (code === FAMILY_HISTORY_DEFINITION.code)
    questionnaire = FAMILY_HISTORY_DEFINITION;
    ...
    questionnaire = SUICIDE_BEHAVIOR_FOLLOWUP_DEFINITION;
  else if (code === PERINATALITE_DEFINITION.code)
    questionnaire = PERINATALITE_DEFINITION;
  else if (code === PATHO_NEURO_DEFINITION.code)
    questionnaire = PATHO_NEURO_DEFINITION;
  else if (code === PATHO_CARDIO_DEFINITION.code)
    questionnaire = PATHO_CARDIO_DEFINITION;
  else if (code === PATHO_ENDOC_DEFINITION.code)
    questionnaire = PATHO_ENDOC_DEFINITION;
  else if (code === PATHO_DERMATO_DEFINITION.code)
    questionnaire = PATHO_DERMATO_DEFINITION;
  else if (code === PATHO_URINAIRE_DEFINITION.code)
    questionnaire = PATHO_URINAIRE_DEFINITION;
  else if (code === ANTECEDENTS_GYNECO_DEFINITION.code)
    questionnaire = ANTECEDENTS_GYNECO_DEFINITION;
  else if (code === PATHO_HEPATO_GASTRO_DEFINITION.code)
    questionnaire = PATHO_HEPATO_GASTRO_DEFINITION;
  else if (code === PATHO_ALLERGIQUE_DEFINITION.code)
    questionnaire = PATHO_ALLERGIQUE_DEFINITION;
  else if (code === AUTRES_PATHO_DEFINITION.code)
    questionnaire = AUTRES_PATHO_DEFINITION;
  else if (code === WAIS4_CRITERIA_DEFINITION.code)
    questionnaire = WAIS4_CRITERIA_DEFINITION;
  else if (code === WAIS4_LEARNING_DEFINITION.code)
    questionnaire = WAIS4_LEARNING_DEFINITION;
  else if (code === WAIS4_MATRICES_DEFINITION.code)
    questionnaire = WAIS4_MATRICES_DEFINITION;
  else if (code === CVLT_DEFINITION.code) questionnaire = CVLT_DEFINITION;
  else if (code === WAIS4_CODE_DEFINITION.code)
    questionnaire = WAIS4_CODE_DEFINITION;
  else if (code === WAIS4_DIGIT_SPAN_DEFINITION.code)
    questionnaire = WAIS4_DIGIT_SPAN_DEFINITION;
  ...
  else if (code === FLUENCES_VERBALES_DEFINITION.code)
    questionnaire = FLUENCES_VERBALES_DEFINITION;
  ...
  else if (code === WAIS4_SIMILITUDES_DEFINITION.code)
    questionnaire = WAIS4_SIMILITUDES_DEFINITION;
  else if (code === TEST_COMMISSIONS_DEFINITION.code)
    questionnaire = TEST_COMMISSIONS_DEFINITION;
  else if (code === SCIP_DEFINITION.code) questionnaire = SCIP_DEFINITION;
  // WAIS-III questionnaires
  else if (code === WAIS3_CVLT_DEFINITION.code)
    questionnaire = WAIS3_CVLT_DEFINITION;
  else if (code === WAIS3_TMT_DEFINITION.code)
    questionnaire = WAIS3_TMT_DEFINITION;
  else if (code === WAIS3_STROOP_DEFINITION.code)
    questionnaire = WAIS3_STROOP_DEFINITION;
  else if (code === WAIS3_FLUENCES_VERBALES_DEFINITION.code)
    questionnaire = WAIS3_FLUENCES_VERBALES_DEFINITION;
  else if (code === WAIS3_CRITERIA_DEFINITION.code)
    questionnaire = WAIS3_CRITERIA_DEFINITION;
  else if (code === WAIS3_LEARNING_DEFINITION.code)
    questionnaire = WAIS3_LEARNING_DEFINITION;
  else if (code === WAIS3_VOCABULAIRE_DEFINITION.code)
    questionnaire = WAIS3_VOCABULAIRE_DEFINITION;
  else if (code === WAIS3_MATRICES_DEFINITION.code)
    questionnaire = WAIS3_MATRICES_DEFINITION;
  else if (code === WAIS3_CODE_SYMBOLES_DEFINITION.code)
    questionnaire = WAIS3_CODE_SYMBOLES_DEFINITION;
  else if (code === WAIS3_DIGIT_SPAN_DEFINITION.code)
    questionnaire = WAIS3_DIGIT_SPAN_DEFINITION;
  else if (code === WAIS3_CPT2_DEFINITION.code)
    questionnaire = WAIS3_CPT2_DEFINITION;
  else if (code === WAIS3_MEM3_SPATIAL_DEFINITION.code)
    questionnaire = WAIS3_MEM3_SPATIAL_DEFINITION;
  ...
  else if (code === FAGERSTROM_DEFINITION.code)
    questionnaire = FAGERSTROM_DEFINITION;
  else if (code === PHYSICAL_PARAMS_DEFINITION.code)
    questionnaire = PHYSICAL_PARAMS_DEFINITION;
  else if (code === BLOOD_PRESSURE_DEFINITION.code)
    questionnaire = BLOOD_PRESSURE_DEFINITION;
  else if (code === SLEEP_APNEA_DEFINITION.code)
    questionnaire = SLEEP_APNEA_DEFINITION;
  else if (code === BIOLOGICAL_ASSESSMENT_DEFINITION.code)
    questionnaire = BIOLOGICAL_ASSESSMENT_DEFINITION;
  else if (code === ECG_DEFINITION.code) questionnaire = ECG_DEFINITION;
  // DSM5 questionnaires
  else if (code === DSM5_HUMEUR_DEFINITION.code)
    questionnaire = DSM5_HUMEUR_DEFINITION;
  else if (code === DSM5_PSYCHOTIC_DEFINITION.code)
    questionnaire = DSM5_PSYCHOTIC_DEFINITION;
  else if (code === DSM5_COMORBID_DEFINITION.code)
    questionnaire = DSM5_COMORBID_DEFINITION;
  // Semi-annual follow-up DSM5 questionnaires
  else if (code === DIAG_PSY_SEM_HUMEUR_ACTUELS_DEFINITION.code)
    questionnaire = DIAG_PSY_SEM_HUMEUR_ACTUELS_DEFINITION;
  else if (code === DIAG_PSY_SEM_HUMEUR_DEPUIS_VISITE_DEFINITION.code)
    questionnaire = DIAG_PSY_SEM_HUMEUR_DEPUIS_VISITE_DEFINITION;
  else if (code === DIAG_PSY_SEM_PSYCHOTIQUES_DEFINITION.code)
    questionnaire = DIAG_PSY_SEM_PSYCHOTIQUES_DEFINITION;
  // Soin, suivi et arrêt de travail questionnaires
  else if (code === SUIVI_RECOMMANDATIONS_DEFINITION.code)
    questionnaire = SUIVI_RECOMMANDATIONS_DEFINITION;
  else if (code === RECOURS_AUX_SOINS_DEFINITION.code)
    questionnaire = RECOURS_AUX_SOINS_DEFINITION;
  else if (code === TRAITEMENT_NON_PHARMACOLOGIQUE_DEFINITION.code)
    questionnaire = TRAITEMENT_NON_PHARMACOLOGIQUE_DEFINITION;
  else if (code === ARRETS_DE_TRAVAIL_DEFINITION.code)
    questionnaire = ARRETS_DE_TRAVAIL_DEFINITION;
  else if (code === SOMATIQUE_CONTRACEPTIF_DEFINITION.code)
    questionnaire = SOMATIQUE_CONTRACEPTIF_DEFINITION;
  else if (code === STATUT_PROFESSIONNEL_DEFINITION.code)
    questionnaire = STATUT_PROFESSIONNEL_DEFINITION;
  // Schizophrenia screening questionnaires
  else if (code === SZ_DIAGNOSTIC_DEFINITION.code)
    questionnaire = SZ_DIAGNOSTIC_DEFINITION;
  else if (code === SZ_ORIENTATION_DEFINITION.code)
    questionnaire = SZ_ORIENTATION_DEFINITION;
  // Schizophrenia initial evaluation questionnaires
  else if (code === SZ_DOSSIER_INFIRMIER_DEFINITION.code)
    questionnaire = SZ_DOSSIER_INFIRMIER_DEFINITION;
  else if (code === SZ_BILAN_BIOLOGIQUE_DEFINITION.code)
    questionnaire = SZ_BILAN_BIOLOGIQUE_DEFINITION;
  // Schizophrenia hetero-questionnaires
    ...
    questionnaire = BRIEF_A_SZ_DEFINITION;
  ...
  // Schizophrenia medical evaluation
    ...
    questionnaire = SZ_PERINATALITE_DEFINITION;
  else if (code === TEA_COFFEE_SZ_DEFINITION.code)
    questionnaire = TEA_COFFEE_SZ_DEFINITION;
  else if (code === EVAL_ADDICTOLOGIQUE_SZ_DEFINITION.code)
    questionnaire = EVAL_ADDICTOLOGIQUE_SZ_DEFINITION;
  // Schizophrenia social module
  else if (code === BILAN_SOCIAL_SZ_DEFINITION.code)
    questionnaire = BILAN_SOCIAL_SZ_DEFINITION;
  ...
  else if (code === EQ5D5L_SZ_DEFINITION.code)
    questionnaire = EQ5D5L_SZ_DEFINITION;
  else if (code === IPAQ_SZ_DEFINITION.code) questionnaire = IPAQ_SZ_DEFINITION;
  else if (code === YBOCS_SZ_DEFINITION.code)
    questionnaire = YBOCS_SZ_DEFINITION;
  else if (code === WURS25_SZ_DEFINITION.code)
    questionnaire = WURS25_SZ_DEFINITION;
  else if (code === STORI_SZ_DEFINITION.code)
    questionnaire = STORI_SZ_DEFINITION;
  ...
  else if (code === PRESENTEISME_SZ_DEFINITION.code)
    questionnaire = PRESENTEISME_SZ_DEFINITION;
  else if (code === FAGERSTROM_SZ_DEFINITION.code)
    questionnaire = FAGERSTROM_SZ_DEFINITION;
  else if (code === BRIEF_A_AUTO_SZ_DEFINITION.code)
    questionnaire = BRIEF_A_AUTO_SZ_DEFINITION;
  else if (code === ONAPS_SZ_DEFINITION.code)
    questionnaire = ONAPS_SZ_DEFINITION;
  else if (code === ALIMENTAIRE_SZ_DEFINITION.code)
    questionnaire = ALIMENTAIRE_SZ_DEFINITION;
  ...
  // Schizophrenia neuropsy module
    ...
    questionnaire = WAIS4_EFFICIENCE_SZ_DEFINITION;
  else if (code === WAIS4_SIMILITUDES_SZ_DEFINITION.code)
    questionnaire = WAIS4_SIMILITUDES_SZ_DEFINITION;
  else if (code === WAIS4_MEMOIRE_CHIFFRES_SZ_DEFINITION.code)
    questionnaire = WAIS4_MEMOIRE_CHIFFRES_SZ_DEFINITION;
  else if (code === WAIS4_MATRICES_SZ_DEFINITION.code)
    questionnaire = WAIS4_MATRICES_SZ_DEFINITION;
  else if (code === SSTICS_SZ_DEFINITION.code)
    questionnaire = SSTICS_SZ_DEFINITION;
  ...
  const useBipolarTables = shouldUseBipolarTables(
    pathology,
    visit?.visit_type,
    code,
  );
  ...
  else if (code === ASRM_DEFINITION.code)
    existingResponse = await getAsrmResponse(visitId);
  else if (code === QIDS_DEFINITION.code)
    existingResponse = await getQidsResponse(visitId);
  else if (code === MDQ_DEFINITION.code)
    existingResponse = await getMdqResponse(visitId);
  else if (code === DIAGNOSTIC_DEFINITION.code)
    existingResponse = await getDiagnosticResponse(visitId);
  else if (code === ORIENTATION_DEFINITION.code)
    existingResponse = await getOrientationResponse(visitId);
  // Initial Evaluation - ETAT questionnaires
  else if (code === EQ5D5L_DEFINITION.code)
    existingResponse = await getEq5d5lResponse(visitId);
  else if (code === PRISE_M_DEFINITION.code)
    existingResponse = await getPriseMResponse(visitId);
  else if (code === STAI_YA_DEFINITION.code)
    existingResponse = await getStaiYaResponse(visitId);
  else if (code === MARS_DEFINITION.code)
    existingResponse = await getMarsResponse(visitId);
  else if (code === MATHYS_DEFINITION.code)
    existingResponse = await getMathysResponse(visitId);
  else if (code === PSQI_DEFINITION.code)
    existingResponse = await getPsqiResponse(visitId);
  else if (code === EPWORTH_DEFINITION.code)
    existingResponse = await getEpworthResponse(visitId);
  // Initial Evaluation - TRAITS questionnaires
  else if (code === ASRS_DEFINITION.code)
    existingResponse = await getAsrsResponse(visitId);
  else if (code === CTQ_DEFINITION.code)
    existingResponse = await getCtqResponse(visitId);
  else if (code === BIS10_DEFINITION.code)
    existingResponse = await getBis10Response(visitId);
  else if (code === ALS18_DEFINITION.code)
    existingResponse = await getAls18Response(visitId);
  else if (code === AIM_DEFINITION.code)
    existingResponse = await getAimResponse(visitId);
  else if (code === WURS25_DEFINITION.code)
    existingResponse = await getWurs25Response(visitId);
  else if (code === AQ12_DEFINITION.code)
    existingResponse = await getAq12Response(visitId);
  else if (code === CSM_DEFINITION.code)
    existingResponse = await getCsmResponse(visitId);
  else if (code === CTI_DEFINITION.code)
    existingResponse = await getCtiResponse(visitId);
  // Hetero questionnaires
  else if (code === MADRS_DEFINITION.code)
    existingResponse = await getMadrsResponse(visitId);
  else if (code === YMRS_DEFINITION.code)
    existingResponse = await getYmrsResponse(visitId);
  else if (code === CGI_DEFINITION.code)
    existingResponse = await getCgiResponse(visitId);
  else if (code === EGF_DEFINITION.code)
    existingResponse = await getEgfResponse(visitId);
  else if (code === ALDA_DEFINITION.code)
    existingResponse = await getAldaResponse(visitId);
  else if (code === ETAT_PATIENT_DEFINITION.code)
    existingResponse = await getEtatPatientResponse(visitId);
  else if (code === FAST_DEFINITION.code)
    existingResponse = await getFastResponse(visitId);
  else if (code === DIVA_DEFINITION.code)
    existingResponse = await getDivaResponse(visitId);
  else if (code === "DIVA_2") existingResponse = await getDivaResponse(visitId);
  else if (code === FAMILY_HISTORY_DEFINITION.code)
    existingResponse = await getFamilyHistoryResponse(visitId);
  else if (code === CSSRS_DEFINITION.code)
    existingResponse = await getCssrsResponse(visitId);
  else if (code === ISA_DEFINITION.code)
    existingResponse = await getIsaResponse(visitId);
  else if (code === ISA_FOLLOWUP_DEFINITION.code)
    ...
    existingResponse = await getSuicideBehaviorFollowupResponse(visitId);
  else if (code === PERINATALITE_DEFINITION.code)
    existingResponse = await getPerinataliteResponse(visitId);
  else if (code === PATHO_NEURO_DEFINITION.code)
    existingResponse = await getPathoNeuroResponse(visitId);
  else if (code === PATHO_CARDIO_DEFINITION.code)
    existingResponse = await getPathoCardioResponse(visitId);
  else if (code === PATHO_ENDOC_DEFINITION.code)
    existingResponse = await getPathoEndocResponse(visitId);
  else if (code === PATHO_DERMATO_DEFINITION.code)
    existingResponse = await getPathoDermatoResponse(visitId);
  else if (code === PATHO_URINAIRE_DEFINITION.code)
    existingResponse = await getPathoUrinaireResponse(visitId);
  else if (code === ANTECEDENTS_GYNECO_DEFINITION.code)
    existingResponse = await getAntecedentsGynecoResponse(visitId);
  else if (code === PATHO_HEPATO_GASTRO_DEFINITION.code)
    existingResponse = await getPathoHepatoGastroResponse(visitId);
  else if (code === PATHO_ALLERGIQUE_DEFINITION.code)
    existingResponse = await getPathoAllergiqueResponse(visitId);
  else if (code === AUTRES_PATHO_DEFINITION.code)
    existingResponse = await getAutresPathoResponse(visitId);
  else if (code === WAIS4_CRITERIA_DEFINITION.code)
    existingResponse = await getWais4CriteriaResponse(visitId);
  else if (code === WAIS4_LEARNING_DEFINITION.code)
    existingResponse = await getWais4LearningResponse(visitId);
  else if (code === WAIS4_MATRICES_DEFINITION.code)
    existingResponse = await getWais4MatricesResponse(visitId);
  else if (code === CVLT_DEFINITION.code)
    existingResponse = await getCvltResponse(visitId);
  else if (code === WAIS4_CODE_DEFINITION.code)
    existingResponse = await getWais4CodeResponse(visitId);
  else if (code === WAIS4_DIGIT_SPAN_DEFINITION.code)
    existingResponse = await getWais4DigitSpanResponse(visitId);
  else if (code === TMT_DEFINITION.code)
    existingResponse = await getTmtResponse(visitId);
  else if (code === STROOP_DEFINITION.code)
    existingResponse = await getStroopResponse(visitId);
  else if (code === FLUENCES_VERBALES_DEFINITION.code)
    existingResponse = await getFluencesVerbalesResponse(visitId);
  else if (code === COBRA_DEFINITION.code)
    existingResponse = await getCobraResponse(visitId);
  else if (code === CPT3_DEFINITION.code)
    existingResponse = await getCpt3Response(visitId);
  else if (code === WAIS4_SIMILITUDES_DEFINITION.code)
    existingResponse = await getWais4SimilitudesResponse(visitId);
  else if (code === TEST_COMMISSIONS_DEFINITION.code)
    existingResponse = await getTestCommissionsResponse(visitId);
  else if (code === SCIP_DEFINITION.code)
    existingResponse = await getScipResponse(visitId);
  // WAIS-III questionnaires
  else if (code === WAIS3_CVLT_DEFINITION.code)
    existingResponse = await getWais3CvltResponse(visitId);
  else if (code === WAIS3_TMT_DEFINITION.code)
    existingResponse = await getWais3TmtResponse(visitId);
  else if (code === WAIS3_STROOP_DEFINITION.code)
    existingResponse = await getWais3StroopResponse(visitId);
  else if (code === WAIS3_FLUENCES_VERBALES_DEFINITION.code)
    existingResponse = await getWais3FluencesVerbalesResponse(visitId);
  else if (code === WAIS3_CRITERIA_DEFINITION.code)
    existingResponse = await getWais3CriteriaResponse(visitId);
  else if (code === WAIS3_LEARNING_DEFINITION.code)
    existingResponse = await getWais3LearningResponse(visitId);
  else if (code === WAIS3_VOCABULAIRE_DEFINITION.code)
    existingResponse = await getWais3VocabulaireResponse(visitId);
  else if (code === WAIS3_MATRICES_DEFINITION.code)
    existingResponse = await getWais3MatricesResponse(visitId);
  else if (code === WAIS3_CODE_SYMBOLES_DEFINITION.code)
    existingResponse = await getWais3CodeSymbolesResponse(visitId);
  else if (code === WAIS3_DIGIT_SPAN_DEFINITION.code)
    existingResponse = await getWais3DigitSpanResponse(visitId);
  else if (code === WAIS3_CPT2_DEFINITION.code)
    existingResponse = await getWais3Cpt2Response(visitId);
  else if (code === WAIS3_MEM3_SPATIAL_DEFINITION.code)
    existingResponse = await getWais3Mem3SpatialResponse(visitId);
  // Social questionnaire
  else if (code === SOCIAL_DEFINITION.code)
    existingResponse = await getSocialResponse(visitId);
  // Infirmier questionnaires
  else if (code === TOBACCO_DEFINITION.code)
    existingResponse = await getTobaccoResponse(visitId);
  else if (code === FAGERSTROM_DEFINITION.code)
    existingResponse = await getFagerstromResponse(visitId);
  else if (code === PHYSICAL_PARAMS_DEFINITION.code)
    existingResponse = await getPhysicalParamsResponse(visitId);
  else if (code === BLOOD_PRESSURE_DEFINITION.code)
    existingResponse = await getBloodPressureResponse(visitId);
  else if (code === SLEEP_APNEA_DEFINITION.code)
    existingResponse = await getSleepApneaResponse(visitId);
  else if (code === BIOLOGICAL_ASSESSMENT_DEFINITION.code)
    existingResponse = await getBiologicalAssessmentResponse(visitId);
  else if (code === ECG_DEFINITION.code)
    existingResponse = await getEcgResponse(visitId);
  // DSM5 questionnaires
  else if (code === DSM5_HUMEUR_DEFINITION.code)
    existingResponse = await getDsm5HumeurResponse(visitId);
  else if (code === DSM5_PSYCHOTIC_DEFINITION.code)
    existingResponse = await getDsm5PsychoticResponse(visitId);
  else if (code === DSM5_COMORBID_DEFINITION.code)
    existingResponse = await getDsm5ComorbidResponse(visitId);
  // Semi-annual follow-up DSM5 questionnaires
  else if (code === DIAG_PSY_SEM_HUMEUR_ACTUELS_DEFINITION.code)
    existingResponse = await getDiagPsySemHumeurActuelsResponse(visitId);
  else if (code === DIAG_PSY_SEM_HUMEUR_DEPUIS_VISITE_DEFINITION.code)
    existingResponse = await getDiagPsySemHumeurDepuisVisiteResponse(visitId);
  else if (code === DIAG_PSY_SEM_PSYCHOTIQUES_DEFINITION.code)
    existingResponse = await getDiagPsySemPsychotiquesResponse(visitId);
  // Soin, suivi et arrêt de travail questionnaires (each has its own table)
  else if (code === SUIVI_RECOMMANDATIONS_DEFINITION.code)
    existingResponse = await getSuiviRecommandationsResponse(visitId);
  else if (code === RECOURS_AUX_SOINS_DEFINITION.code)
    existingResponse = await getRecoursAuxSoinsResponse(visitId);
  else if (code === TRAITEMENT_NON_PHARMACOLOGIQUE_DEFINITION.code)
    existingResponse = await getTraitementNonPharmaResponse(visitId);
  else if (code === ARRETS_DE_TRAVAIL_DEFINITION.code)
    existingResponse = await getArretsTravailResponse(visitId);
  else if (code === SOMATIQUE_CONTRACEPTIF_DEFINITION.code)
    existingResponse = await getSomatiqueContraceptifResponse(visitId);
  else if (code === STATUT_PROFESSIONNEL_DEFINITION.code)
    existingResponse = await getStatutProfessionnelResponse(visitId);
  // Schizophrenia screening questionnaires
  else if (code === SZ_DIAGNOSTIC_DEFINITION.code)
    existingResponse = await getScreeningSzDiagnosticResponse(visitId);
  else if (code === SZ_ORIENTATION_DEFINITION.code)
    existingResponse = await getScreeningSzOrientationResponse(visitId);
  // Schizophrenia initial evaluation questionnaires
  else if (code === SZ_DOSSIER_INFIRMIER_DEFINITION.code)
    existingResponse = await getDossierInfirmierSzResponse(visitId);
  else if (code === SZ_BILAN_BIOLOGIQUE_DEFINITION.code)
    existingResponse = await getBilanBiologiqueSzResponse(visitId);
  // Schizophrenia hetero-questionnaires
  else if (code === PANSS_DEFINITION.code)
    existingResponse = await getPanssResponse(visitId);
  else if (code === CDSS_DEFINITION.code)
    existingResponse = await getCdssResponse(visitId);
  else if (code === BARS_DEFINITION.code)
    existingResponse = await getBarsResponse(visitId);
  else if (code === SUMD_DEFINITION.code)
    existingResponse = await getSumdResponse(visitId);
  else if (code === AIMS_DEFINITION.code)
    existingResponse = await getAimsResponse(visitId);
  else if (code === BARNES_DEFINITION.code)
    ...
    existingResponse = await getBriefASzResponse(visitId);
  else if (code === YMRS_SZ_DEFINITION.code)
    existingResponse = await getSchizophreniaInitialResponse(
      "YMRS_SZ",
      visitId,
    );
  else if (code === CGI_SZ_DEFINITION.code)
    existingResponse = await getSchizophreniaInitialResponse("CGI_SZ", visitId);
  else if (code === EGF_SZ_DEFINITION.code)
    existingResponse = await getSchizophreniaInitialResponse("EGF_SZ", visitId);
  // Schizophrenia medical evaluation
    ...
    existingResponse = await getPerinataliteSzResponse(visitId);
  else if (code === TEA_COFFEE_SZ_DEFINITION.code)
    existingResponse = await getTeaCoffeeSzResponse(visitId);
  else if (code === EVAL_ADDICTOLOGIQUE_SZ_DEFINITION.code)
    existingResponse = await getEvalAddictologiqueSzResponse(visitId);
  // Schizophrenia social module
  else if (code === BILAN_SOCIAL_SZ_DEFINITION.code)
    existingResponse = await getBilanSocialSzResponse(visitId);
  // Schizophrenia auto module (patient self-administered)
  else if (code === SQOL_SZ_DEFINITION.code)
    existingResponse = await getSqolResponse(visitId);
  else if (code === CTQ_SZ_DEFINITION.code)
    existingResponse = await getCtqSzResponse(visitId);
  else if (code === MARS_SZ_DEFINITION.code)
    existingResponse = await getMarsSzResponse(visitId);
  else if (code === BIS_SZ_DEFINITION.code)
    existingResponse = await getBisSzResponse(visitId);
  else if (code === EQ5D5L_SZ_DEFINITION.code)
    existingResponse = await getEq5d5lSzResponse(visitId);
  else if (code === IPAQ_SZ_DEFINITION.code)
    existingResponse = await getIpaqSzResponse(visitId);
  else if (code === YBOCS_SZ_DEFINITION.code)
    existingResponse = await getYbocsSzResponse(visitId);
  else if (code === WURS25_SZ_DEFINITION.code)
    existingResponse = await getWurs25SzResponse(visitId);
  else if (code === STORI_SZ_DEFINITION.code)
    existingResponse = await getStoriSzResponse(visitId);
  else if (code === SOGS_SZ_DEFINITION.code)
    existingResponse = await getSogsSzResponse(visitId);
  else if (code === PSQI_SZ_DEFINITION.code)
    existingResponse = await getPsqiSzResponse(visitId);
  else if (code === PRESENTEISME_SZ_DEFINITION.code)
    existingResponse = await getPresenteismeSzResponse(visitId);
  else if (code === FAGERSTROM_SZ_DEFINITION.code)
    existingResponse = await getFagerstromSzResponse(visitId);
  else if (code === BRIEF_A_AUTO_SZ_DEFINITION.code)
    existingResponse = await getSchizophreniaInitialResponse("BRIEF_A_AUTO_SZ", visitId);
  else if (code === ONAPS_SZ_DEFINITION.code)
    existingResponse = await getOnapsSzResponse(visitId);
  else if (code === ALIMENTAIRE_SZ_DEFINITION.code)
    existingResponse = await getAlimentaireSzResponse(visitId);
  // Schizophrenia entourage module (caregiver-administered)
  else if (code === EPHP_SZ_DEFINITION.code)
    existingResponse = await getEphpSzResponse(visitId);
  // Schizophrenia neuropsy module
  else if (code === SZ_CVLT_DEFINITION.code)
    existingResponse = await getCvltSzResponse(visitId);
  else if (code === TMT_SZ_DEFINITION.code)
    existingResponse = await getTmtSzResponse(visitId);
  else if (code === COMMISSIONS_SZ_DEFINITION.code)
    ...
    existingResponse = await getWais4EfficienceSzResponse(visitId);
  else if (code === WAIS4_SIMILITUDES_SZ_DEFINITION.code)
    existingResponse = await getWais4SimilitudesSzResponse(visitId);
  else if (code === WAIS4_MEMOIRE_CHIFFRES_SZ_DEFINITION.code)
    existingResponse = await getWais4MemoireChiffresSzResponse(visitId);
  else if (code === WAIS4_MATRICES_SZ_DEFINITION.code)
    existingResponse = await getWais4MatricesSzResponse(visitId);
  else if (code === SSTICS_SZ_DEFINITION.code)
    existingResponse = await getSsticsSzResponse(visitId);
  else if (code === CBQ_SZ_DEFINITION.code)
  ...
  if (code === "PSQI_SZ") {
    console.log(
      "[Page.tsx] PSQI_SZ existingResponse:",
      existingResponse ? "Found" : "Not found",
      existingResponse?.total_score,
    );
  }
  ...
  if (code === "WAIS4_MEMOIRE_CHIFFRES_SZ") {
    console.log(
      "[Page.tsx] WAIS4_MEMOIRE_CHIFFRES_SZ existingResponse:",
      existingResponse ? "Found" : "Not found",
    );
    if (existingResponse) {
  ...
  if (existingResponse) {
    ...
    initialResponses = normalizeResponseForQuestionnaireForm(
      questionnaire,
      initialResponses,
  ...
  if (code === "SLEEP_APNEA" && existingResponse) {
    ...
    initialResponses.has_cpap_device = boolToYesNo(
      initialResponses.has_cpap_device,
    );
    ...
    initialResponses.observed_apnea = boolToYesNo(
      initialResponses.observed_apnea,
    );
  ...
  if (code === "CSSRS" && existingResponse) {
    ...
    initialResponses.q2_non_specific = boolToNum(
      initialResponses.q2_non_specific,
    );
    initialResponses.q3_method_no_intent = boolToNum(
      initialResponses.q3_method_no_intent,
    );
    initialResponses.q4_intent_no_plan = boolToNum(
      initialResponses.q4_intent_no_plan,
    );
    initialResponses.q5_plan_intent = boolToNum(
      initialResponses.q5_plan_intent,
    );
  }

  // Convert boolean to number for WAIS Criteria acceptance field
  if (
    (code === "WAIS3_CRITERIA" || code === "WAIS4_CRITERIA") &&
    existingResponse
  ) {
    const boolToNum = (val: any): number | null => {
      if (val === true) return 1;
    ...
    initialResponses.accepted_for_neuropsy_evaluation = boolToNum(
      initialResponses.accepted_for_neuropsy_evaluation,
    );
    console.log(
      "[WAIS Debug] Converted accepted_for_neuropsy_evaluation from boolean to:",
      initialResponses.accepted_for_neuropsy_evaluation,
    );
  }
  ...
  if (code === "BILAN_SOCIAL_SZ" && existingResponse) {
    ...
    initialResponses.justice_safeguard = boolToNum(
      initialResponses.justice_safeguard,
    );
  }
  ...
  if (code === "WAIS4_SIMILITUDES" && existingResponse) {
    ...
    console.log(
      "[WAIS4 Similitudes Debug] Removed calculated score fields from form",
    );
  }
  ...
  if (code === "WAIS4_MATRICES" && existingResponse) {
    ...
    console.log(
      "[WAIS4 Matrices Debug] Removed calculated score fields from form",
    );
  }
  ...
  if (code === "WAIS4_DIGIT_SPAN" && existingResponse) {
    ...
    delete initialResponses.standardized_score;
    console.log(
      "[WAIS4 Digit Span Debug] Removed calculated score fields from form",
    );
  }
  ...
  if (code === "FLUENCES_VERBALES" && existingResponse) {
    ...
    console.log(
      "[Fluences Verbales Debug] Removed calculated score fields from form",
    );
  }
  ...
  if (requiresDemographics) {
    ...
    console.log(
      "[Demographics Debug] Patient:",
      patient
        ? { id: patient.id, dob: patient.date_of_birth, gender: patient.gender }
        : "NOT FOUND",
    );
    console.log(
      "[Demographics Debug] Visit:",
      visit
        ? { id: visit.id, scheduled_date: visit.scheduled_date }
        : "NOT FOUND",
    );
    ...
    if (patient) {
      ...
      if (patient.date_of_birth) {
        ...
        const calculatedAge = calculateAgeAtDate(
          patient.date_of_birth,
          referenceDate,
        );
        console.log(
          "[Demographics Debug] Calculated age:",
          calculatedAge,
          "from DOB:",
          patient.date_of_birth,
          "ref date:",
        ...
        // BRIEF-A uses subject_age field (hetero informant about the subject)
        if (code === "BRIEF_A_SZ") {
          initialResponses = {
            ...initialResponses,
            subject_age: calculatedAge,
          };
        }
        ...
        if (questionnaireUsesAgeField(code)) {
          ...
          console.log(
            '[Demographics Debug] Also injecting into "age" field for criteria questionnaire',
          );
        }
        ...
        if (questionnaireUsesNeuroAgeField(code)) {
          ...
          console.log(
            '[Demographics Debug] Also injecting into "neuro_age" field for SZ WAIS-IV questionnaire',
          );
        }
      } else {
      ...
      if (patient.gender) {
        ...
        initialResponses = {
          ...initialResponses,
          patient_gender: normalizedGender || patient.gender,
        };

        // BRIEF-A uses subject_sex field
        if (code === "BRIEF_A_SZ") {
          initialResponses = {
            ...initialResponses,
            subject_sex: normalizedGender,
          };
        }
      ...
      if (
        patient.years_of_education !== null &&
        patient.years_of_education !== undefined
      ) {
        console.log(
          "[Demographics Debug] Years of education:",
        ...
        initialResponses = {
          ...initialResponses,
          years_of_education: patient.years_of_education,
        };
        ...
        if (code === "WAIS3_DIGIT_SPAN") {
          ...
          console.log(
            "[Demographics Debug] Converted to education_level category:",
            educationLevel,
          );
        }
        ...
        if (questionnaireUsesAnneesEtudesField(code)) {
          initialResponses = {
            ...initialResponses,
            annees_etudes: patient.years_of_education,
          };
          console.log(
            '[Demographics Debug] Also injecting into "annees_etudes" field for SZ WAIS-IV questionnaire',
          );
        }
      } else {
        ...
        if (code === "WAIS3_CRITERIA" || code === "WAIS4_CRITERIA") {
          ...
          console.log(
            "[Demographics Debug] Years of education not set in patient profile (will show as empty)",
          );
        }
        ...
        if (questionnaireUsesAnneesEtudesField(code)) {
          ...
          console.log(
            "[Demographics Debug] Annees etudes not set in patient profile (will show as empty)",
          );
        }
      }
    }
    ...
    console.log(
      "[Demographics Debug] Final initialResponses keys:",
  ...
  if (code === "WAIS3_CRITERIA" || code === "WAIS4_CRITERIA") {
    if (!initialResponses.collection_date) {
      ...
      console.log(
        "[Demographics Debug] Set default collection_date to today:",
        today,
      );
    }
  }
  ...
  if (code === "WAIS4_CRITERIA_SZ") {
    if (!initialResponses.date_neuropsychologie) {
      ...
      console.log(
        "[Demographics Debug] Set default date_neuropsychologie to today:",
        today,
      );
    }
  }
  ...
  if (code === BIOLOGICAL_ASSESSMENT_DEFINITION.code) {
    ...
    if (physicalParams?.weight_kg) {
      initialResponses = {
        ...initialResponses,
        weight_kg: physicalParams.weight_kg,
      };
      console.log(
        "[Creatinine Debug] Added weight_kg to initialResponses:",
        initialResponses.weight_kg,
      );
    }
    console.log(
      "[Creatinine Debug] Final initialResponses for bio assessment:",
      {
  ...
  if (
    code === SZ_DIAGNOSTIC_DEFINITION.code &&
    !initialResponses.screening_diag_nommed
  ) {
    const patient = await getPatientById(patientId);
    if (
      patient &&
      patient.assigned_to_first_name &&
      patient.assigned_to_last_name
    ) {
      initialResponses = {
        ...initialResponses,
  ...
  if (code === "WAIS4_DIGIT_SPAN") {
    ...
    console.log(
      "[WAIS4 Digit Span Debug] Filtered out score sections. Original questions:",
  ...
  if (code === "CVLT") {
    ...
    console.log(
      "[CVLT Debug] Filtered out score fields. Original questions:",
      serializableQuestionnaire.questions.length,
      "Filtered:",
      filteredQuestionnaire.questions.length,
    );
