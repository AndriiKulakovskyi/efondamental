...
export function ScoreDisplay({ code: rawCode, data }: ScoreDisplayProps) {
  ...
  const formatBriefNorm = (t: any, p: any): string => {
    const hasT = !(t === null || t === undefined);
    const hasP = !(p === null || p === undefined);
    if (!hasT && !hasP) return "-";
    if (hasT && hasP) return `T=${t} (P${p})`;
    if (hasT) return `T=${t}`;
    return `P${p}`;
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm mt-1">
              Ce questionnaire est un recueil d'informations et ne comporte pas
              de calcul de score.
            </p>
          </div>
        </div>
  ...
  const getSeverity = () => {
    ...
    if (code === "MDQ") {
      ...
      const isPositive =
        data.interpretation?.includes("Positif") ||
    ...
    if (code === "ASRS") {
      ...
      const isPositive =
        data.screening_positive || data.part_a_positive_items >= 4;
      return isPositive ? "warning" : "info";
    }
    ...
    if (code === "CSM") {
      ...
      if (
        data.chronotype === "definitely_morning" ||
        data.chronotype === "definitely_evening"
      )
        return "info";
      if (
        data.chronotype === "moderately_morning" ||
        data.chronotype === "moderately_evening"
      )
        return "info";
    ...
    if (code === "WAIS4_DIGIT_SPAN" || code === "WAIS4_MEMOIRE_CHIFFRES_SZ") {
    ...
    if (code === "BRIEF_A_AUTO_SZ") {
      // BRIEF-A Auto: GEC score based (75-225) - same thresholds as hetero
      // Higher scores indicate more executive dysfunction
      if (data.brief_a_gec > 187) return "error";
      if (data.brief_a_gec > 150) return "warning";
      if (data.brief_a_gec > 112) return "info";
  ...
  if (
    ...
    if (data.standard_score >= 13) {
      ...
      interpretation =
        "Raisonnement verbal significativement inférieur à la moyenne";
    }
  }
  ...
  if (
    ...
    if (data.wais_mat_std >= 13) {
      ...
      interpretation =
        "Intelligence fluide significativement inférieure à la moyenne";
    }
  }
  ...
  if (
    ...
    if (data.sstics_scorez >= 1.5) {
      interpretation =
        "Plaintes cognitives nettement inférieures à la population de référence";
    } else if (data.sstics_scorez >= 0.5) {
      interpretation =
        "Plaintes cognitives inférieures à la population de référence";
    } else if (data.sstics_scorez >= -0.5) {
      interpretation =
        "Plaintes cognitives comparables à la population de référence";
    } else if (data.sstics_scorez >= -1.5) {
      interpretation =
        "Plaintes cognitives supérieures à la population de référence";
    } else {
      interpretation =
        "Plaintes cognitives nettement supérieures à la population de référence";
    }
  }
  ...
  if (
    ...
    if (data.brief_a_gec > 187) {
      interpretation = "Difficultés exécutives sévères (auto-rapporté)";
    } else if (data.brief_a_gec > 150) {
      interpretation = "Difficultés exécutives modérées (auto-rapporté)";
    } else if (data.brief_a_gec > 112) {
      interpretation = "Difficultés exécutives légères (auto-rapporté)";
    } else {
      interpretation = "Fonctions exécutives dans la norme (auto-rapporté)";
    }
  }
  ...
  if (
    ...
    if (data.standard_score >= 13) {
      ...
      interpretation =
        "Connaissances lexicales significativement inférieures à la moyenne";
    }
  }
  ...
  if (
    ...
    if (data.standardized_score >= 13) {
      ...
      interpretation =
        "Raisonnement perceptif significativement inférieur à la moyenne";
    }
  }
  ...
  if (
    ...
    if (data.wais_mc_std >= 13) {
      ...
      interpretation =
        "Mémoire de travail significativement inférieure à la moyenne";
    }
  }
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold">
              ...
              {code === "EGF_SZ" &&
                "Résultats EGF - Échelle Globale de Fonctionnement"}
              {code === "WAIS4_MATRICES" && "Résultats WAIS-IV Matrices"}
              {code === "WAIS4_MATRICES_SZ" && "Résultats WAIS-IV Matrices"}
              {code === "SSTICS_SZ" &&
                "Résultats SSTICS - Plaintes Cognitives Subjectives"}
              {code === "CBQ_SZ" && "Résultats CBQ - Biais Cognitifs"}
              {code === "DACOBS_SZ" &&
                "Résultats DACOBS - Biais Cognitifs et Comportements"}
              {code === "BRIEF_A_SZ" &&
                "Résultats BRIEF-A - Fonctions Exécutives"}
              {code === "BRIEF_A_AUTO_SZ" &&
                "Résultats BRIEF-A Auto-Evaluation - Fonctions Exécutives"}
              {(code === "WAIS4_DIGIT_SPAN" ||
                code === "WAIS4_MEMOIRE_CHIFFRES_SZ") &&
                "Résultats WAIS-IV Mémoire des chiffres (Digit Span)"}
              {(code === "WAIS4_SIMILITUDES" ||
                code === "WAIS4_SIMILITUDES_SZ") &&
                "Résultats WAIS-IV Similitudes"}
              ...
              {code === "PSP" &&
                "Résultats PSP - Fonctionnement personnel et social"}
              {(code === "EQ5D5L" || code === "EQ5D5L_SZ") &&
                "Résultats EQ-5D-5L - Qualité de vie"}
              ...
              {(code === "MARS" || code === "MARS_SZ") &&
                "Résultats MARS - Observance thérapeutique"}
              {code === "BIS_SZ" &&
                "Résultats BIS - Échelle d'Insight de Birchwood"}
              {code === "YBOCS_SZ" &&
                "Résultats Y-BOCS - Obsessions-Compulsions"}
              {code === "SOGS_SZ" && "Résultats SOGS - Jeu Pathologique"}
              {code === "PSQI_SZ" && "Résultats PSQI - Qualité du Sommeil"}
              {code === "PRESENTEISME_SZ" &&
                "Résultats WHO-HPQ - Absentéisme et Présentéisme"}
              ...
              {code === "FAGERSTROM_SZ" &&
                "Résultats FTND - Test de Fagerström"}
              {code === "EPHP_SZ" &&
                "Résultats EPHP - Handicap Psychique (Entourage)"}
            </h4>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
              <p className="text-blue-900 text-xs leading-relaxed">
                L'échelle d'Alda est un outil rétrospectif conçu pour estimer
                dans quelle mesure l'amélioration clinique peut être attribuée
                spécifiquement au lithium, en tenant compte des facteurs de
                confusion (observance, traitements concomitants, évolution
                naturelle).
              </p>
            </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-base">
                    {data.alda_score !== null &&
                      data.alda_score >= 7 &&
                      "Bon répondeur au lithium"}
                    {data.alda_score !== null &&
                      data.alda_score >= 4 &&
                      data.alda_score <= 6 &&
                      "Réponse partielle / intermédiaire"}
                    {data.alda_score !== null &&
                      data.alda_score >= 0 &&
                      data.alda_score <= 3 &&
                      "Mauvais ou non-répondeur"}
                  </span>
                  <span className="text-xs text-gray-600 font-medium">
                    Score Total: {data.alda_score ?? "-"}/10
                  </span>
                </div>
                  ...
                  <p className="text-xs leading-relaxed text-gray-700">
                    {data.alda_score !== null && data.alda_score >= 7 && (
                      <>
                        ...
                        avec peu de facteurs confondants. Profil typique de
                        "lithium responder". Le lithium doit être maintenu comme
                      ...
                      data.alda_score <= 6 && (
                        <>
                          ...
                          <strong className="text-blue-700">
                            probable mais attribution partielle
                          </strong>{" "}
                          au lithium. Peut justifier une poursuite ou une
                          optimisation du traitement (dosage, observance).
                          Évaluer si le lithium reste pertinent en association.
                        </>
                      )}
                    {data.alda_score !== null &&
                      data.alda_score >= 0 &&
                      data.alda_score <= 3 && (
                        <>
                          ...
                          ou amélioration non attribuable au lithium. Intérêt
                          limité du lithium en monothérapie. Considérer des
                          alternatives thérapeutiques ou une réévaluation
                          diagnostique.
                        </>
                      )}
                  </p>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-gray-700 font-medium">
                      Score A - Réponse clinique au lithium:
                    </span>
                    <span className="font-bold text-lg">
                      {data.qa ?? data.score_a ?? "-"}/10
                    </span>
                  </div>
                  <p className="text-xs text-gray-600">
                    Jugement clinique synthétique basé sur la réduction de la
                    fréquence et sévérité des épisodes, l'amélioration du
                    fonctionnement, et la stabilité sous lithium.
                  </p>
                  <div className="text-xs text-gray-500 mt-2">
                    <p>
                      0 = aucune réponse | 1-3 = minime | 4-6 = modérée | 7-9 =
                      marquée | 10 = excellente
                    </p>
                  </div>
                </div>
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-gray-700 font-medium">
                      Score B - Facteurs confondants:
                    </span>
                    <span className="font-bold text-lg">
                      {data.b_total_score ?? data.score_b ?? "-"}/10
                    </span>
                  </div>
                  <p className="text-xs text-gray-600 mb-2">
                    Plus le score B est élevé, moins l'amélioration peut être
                    attribuée au lithium seul.
                  </p>
                  <div className="grid grid-cols-1 gap-1 text-xs">
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        B1 - Observance insuffisante:
                      </span>
                      <span className="font-medium">{data.qb1 ?? "-"}/2</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        B2 - Traitements concomitants actifs:
                      </span>
                      <span className="font-medium">{data.qb2 ?? "-"}/2</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        B3 - Durée insuffisante de traitement:
                      </span>
                      <span className="font-medium">{data.qb3 ?? "-"}/2</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        B4 - Évolution naturelle fluctuante:
                      </span>
                      <span className="font-medium">{data.qb4 ?? "-"}/2</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">
                        B5 - Données cliniques insuffisantes:
                      </span>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t space-y-1">
              ...
              <p>
                • <strong>7-10 :</strong> Bon répondeur - Réponse robuste,
                maintenir le lithium
              </p>
              <p>
                • <strong>4-6 :</strong> Réponse partielle - Optimisation ou
                association possible
              </p>
              <p>
                • <strong>0-3 :</strong> Non-répondeur - Alternatives
                thérapeutiques à considérer
              </p>
            </div>
          </div>
        )}
        ...
        {(code === "CGI" || code === "CGI_SZ") && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            {data.therapeutic_index !== undefined &&
              data.therapeutic_index !== null && (
                <div className="pt-2 border-t">
                  <h5 className="font-semibold text-gray-700 mb-1">
        ...
        {code === "ASRS" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="flex justify-between pt-2 border-t">
              ...
              <span
                className={`font-bold text-lg ${data.screening_positive ? "text-orange-700" : "text-blue-700"}`}
              >
                {data.screening_positive ? "POSITIF" : "Négatif"}
              </span>
            </div>
            {data.screening_positive && (
              <p className="text-xs text-gray-600 mt-1 pt-2 border-t">
                Un résultat positif suggère des symptômes cohérents avec un TDAH
                chez l'adulte. Une évaluation clinique complète est recommandée.
              </p>
            )}
          </div>
        )}
          ...
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">
                    {data.emotional_abuse_score ?? "-"}/25
                  </span>
                  <Badge
                    variant={
                      data.emotional_abuse_severity?.includes("severe")
                        ? "destructive"
                        : "secondary"
                    }
                    className={`text-xs ${data.emotional_abuse_severity?.includes("moderate") ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}
                  >
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">
                    {data.physical_abuse_score ?? "-"}/25
                  </span>
                  <Badge
                    variant={
                      data.physical_abuse_severity?.includes("severe")
                        ? "destructive"
                        : "secondary"
                    }
                    className={`text-xs ${data.physical_abuse_severity?.includes("moderate") ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}
                  >
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">
                    {data.sexual_abuse_score ?? "-"}/25
                  </span>
                  <Badge
                    variant={
                      data.sexual_abuse_severity?.includes("severe")
                        ? "destructive"
                        : "secondary"
                    }
                    className={`text-xs ${data.sexual_abuse_severity?.includes("moderate") ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}
                  >
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">
                    {data.emotional_neglect_score ?? "-"}/25
                  </span>
                  <Badge
                    variant={
                      data.emotional_neglect_severity?.includes("severe")
                        ? "destructive"
                        : "secondary"
                    }
                    className={`text-xs ${data.emotional_neglect_severity?.includes("moderate") ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}
                  >
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">
                    {data.physical_neglect_score ?? "-"}/25
                  </span>
                  <Badge
                    variant={
                      data.physical_neglect_severity?.includes("severe")
                        ? "destructive"
                        : "secondary"
                    }
                    className={`text-xs ${data.physical_neglect_severity?.includes("moderate") ? "bg-orange-100 text-orange-700 border-orange-200" : ""}`}
                  >
            ...
            <div className="flex justify-between pt-2 border-t">
              ...
              <span
                className={`font-bold ${data.denial_score >= 6 ? "text-orange-700" : "text-green-700"}`}
              >
            ...
            {data.denial_score >= 6 && (
              <p className="text-xs text-orange-700 mt-1 pt-2 border-t">
                Score de minimisation significatif (≥6) : suspicion de
                sous-déclaration des traumatismes. Les résultats doivent être
          ...
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            {data.overall_impulsivity >= 3.0 && (
              <p className="text-xs text-red-700 mt-1 pt-2 border-t">
                Niveau d'impulsivité élevé. Une évaluation clinique plus
                approfondie peut être recommandée.
              </p>
            )}
            {data.overall_impulsivity >= 2.5 &&
              data.overall_impulsivity < 3.0 && (
                <p className="text-xs text-orange-700 mt-1 pt-2 border-t">
        ...
        {code === "WURS25" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="flex justify-between pt-2 border-t">
              ...
              <span
                className={`font-bold text-lg ${data.adhd_likely ? "text-orange-700" : "text-blue-700"}`}
              >
            ...
            {data.adhd_likely && (
              <p className="text-xs text-orange-700 mt-1 pt-2 border-t">
                Le score suggère la présence de symptômes de TDAH dans l'enfance
                (avant 12 ans). Une évaluation clinique complète est recommandée
                pour le diagnostic de TDAH chez l'adulte.
              </p>
            )}
            {!data.adhd_likely && (
              <p className="text-xs text-blue-700 mt-1 pt-2 border-t">
                Le score ne suggère pas de symptômes significatifs de TDAH dans
                l'enfance.
              </p>
            )}
          </div>
          ...
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
                ...
                <div>
                  <span className="font-medium">Impulsivité/Colère:</span> Q7,
                  Q9, Q17, Q21, Q24, Q27
                </div>
                <div>
                  <span className="font-medium">Anxiété/Humeur:</span> Q4, Q12,
                  Q20
                </div>
                <div>
                  <span className="font-medium">Comportement:</span> Q11, Q15,
                  Q25, Q28, Q41
                </div>
                <div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>WURS-25:</strong> Échelle de Wender Utah - Évaluation
                rétrospective du TDAH dans l'enfance
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 25 items (0-4). Score total: 0-100.
              </p>
              <p className="mt-1">
                <strong>Seuil clinique:</strong> ≥46 (sensibilité/spécificité:
                96%)
              </p>
              <p className="mt-1">
                <strong>Référence:</strong> Ward MF et al. Am J Psychiatry. 1993
              </p>
            </div>
          </div>
        )}
        ...
        {code === "STORI_SZ" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="space-y-2 pt-2 border-t">
              <h5 className="font-semibold text-gray-700 text-xs">
                Scores par stade (0-50)
              </h5>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-3 relative">
                  <div
                    ...
                    style={{
                      width: `${((data.stori_etap1 ?? 0) / 50) * 100}%`,
                    }}
                  />
                </div>
                <span
                  className={`text-xs font-semibold w-8 text-right ${data.dominant_stage === 1 ? "text-red-600" : "text-gray-600"}`}
                >
                  {data.stori_etap1 ?? "-"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-3 relative">
                  <div
                    ...
                    style={{
                      width: `${((data.stori_etap2 ?? 0) / 50) * 100}%`,
                    }}
                  />
                </div>
                <span
                  className={`text-xs font-semibold w-8 text-right ${data.dominant_stage === 2 ? "text-orange-600" : "text-gray-600"}`}
                >
                  {data.stori_etap2 ?? "-"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-3 relative">
                  <div
                    ...
                    style={{
                      width: `${((data.stori_etap3 ?? 0) / 50) * 100}%`,
                    }}
                  />
                </div>
                <span
                  className={`text-xs font-semibold w-8 text-right ${data.dominant_stage === 3 ? "text-yellow-600" : "text-gray-600"}`}
                >
                  {data.stori_etap3 ?? "-"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-3 relative">
                  <div
                    ...
                    style={{
                      width: `${((data.stori_etap4 ?? 0) / 50) * 100}%`,
                    }}
                  />
                </div>
                <span
                  className={`text-xs font-semibold w-8 text-right ${data.dominant_stage === 4 ? "text-lime-600" : "text-gray-600"}`}
                >
                  {data.stori_etap4 ?? "-"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 bg-gray-200 rounded-full h-3 relative">
                  <div
                    ...
                    style={{
                      width: `${((data.stori_etap5 ?? 0) / 50) * 100}%`,
                    }}
                  />
                </div>
                <span
                  className={`text-xs font-semibold w-8 text-right ${data.dominant_stage === 5 ? "text-green-600" : "text-gray-600"}`}
                >
                  {data.stori_etap5 ?? "-"}
                </span>
              </div>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="space-y-1 text-xs text-gray-600">
                <p>
                  ...
                  Repli, perte, confusion, impuissance
                </p>
                <p>
                ...
                <p>
                  ...
                  Travail actif vers un style de vie positif
                </p>
                <p>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>STORI:</strong> Stages of Recovery Instrument -
                Évaluation du rétablissement psychologique
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 50 items (0-5). 10 items par stade.
                Score par stade: 0-50.
              </p>
              <p className="mt-1">
                <strong>Modèle:</strong> Andresen Recovery Model (5 stades)
              </p>
              <p className="mt-1">
                <strong>Référence:</strong> Andresen R et al. Aust N Z J
                Psychiatry. 2006
              </p>
            </div>
          </div>
        ...
        {code === "SOGS_SZ" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              ...
              <div className="space-y-1 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    <strong>0-2:</strong> Pas de problème de jeu
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-orange-500" />
                  <span>
                    <strong>3-4:</strong> Joueur à risque
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span>
                    <strong>5+:</strong> Joueur pathologique probable
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>SOGS:</strong> South Oaks Gambling Screen - Dépistage du
                jeu pathologique
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 20 items cotés (11 base + 9
                conditionnels)
              </p>
              <p className="mt-1">
                <strong>Référence:</strong> Lesieur HR, Blume SB. Am J
                Psychiatry. 1987 / Adaptation: Lejoyeux 1999
              </p>
            </div>
          </div>
        )}
        ...
        {code === "PSQI_SZ" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              <h5 className="font-semibold text-gray-700 mb-2 text-xs">
                Seuils cliniques (score global)
              </h5>
              <div className="space-y-1 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-500" />
                  <span>
                    <strong>0-5:</strong> Bonne qualité de sommeil
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-orange-500" />
                  <span>
                    <strong>6-10:</strong> Qualité altérée
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span>
                    <strong>11-21:</strong> Mauvaise qualité de sommeil
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>PSQI:</strong> Pittsburgh Sleep Quality Index - Indice
                de qualité du sommeil
              </p>
              <p className="mt-1">
                <strong>Seuil clinique:</strong> Score &gt;5 indique une
                mauvaise qualité de sommeil (sensibilité ~90%, spécificité ~86%)
              </p>
              <p className="mt-1">
                <strong>Référence:</strong> Buysse DJ et al. Psychiatry Res.
                1989
              </p>
            </div>
          </div>
        ...
        {code === "PRESENTEISME_SZ" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            {/* Absenteeism Section */}
            <div className="space-y-2">
              <h5 className="font-semibold text-gray-700 text-xs border-b pb-1">
                Absentéisme
              </h5>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="space-y-1 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-green-500" />
                  <span>
                    <strong>Productivité ≥80%:</strong> Niveau satisfaisant
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-orange-500" />
                  <span>
                    <strong>Productivité 60-79%:</strong> Réduction modérée
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-red-500" />
                  <span>
                    <strong>Productivité &lt;60%:</strong> Réduction
                    significative
                  </span>
                </div>
              </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>WHO-HPQ:</strong> World Health Organization Health and
                Work Performance Questionnaire
              </p>
              <p className="mt-1">
                <strong>Absentéisme absolu:</strong> Jours d&apos;absence pour
                raison de santé (B5a + B5c)
              </p>
              <p className="mt-1">
                <strong>Performance relative:</strong> Auto-évaluation vs
                collègues (B11 - B9)
              </p>
              <p className="mt-1">
                <strong>Perte de performance:</strong> Déclin récent vs
                historique (B10 - B11)
              </p>
              <p className="mt-1">
                <strong>Référence:</strong> Kessler RC et al. J Occup Environ
                Med. 2003
              </p>
            </div>
          </div>
        ...
        {code === "CSM" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="text-xs text-gray-600 mt-2 pt-2 border-t space-y-1">
              <p>
                <strong>Echelle:</strong> 13-21 (vespéral), 22-28 (mod.
                vespéral), 29-41 (intermédiaire), 42-47 (mod. matinal), 48-55
                (matinal)
              </p>
              <p>
                Le chronotype peut être perturbé dans le trouble bipolaire. Les
                types vespéraux sont souvent associés aux épisodes dépressifs.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 border-t">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {data.flexibility_score ?? "-"}/25
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Languide:</span>
                <span className="font-semibold">
                  {data.languid_score ?? "-"}/30
                </span>
              </div>
            </div>
            <div className="flex justify-between pt-2 border-t">
            ...
            <div className="text-xs text-gray-600 mt-2 pt-2 border-t space-y-1">
              <p>
                <strong>Echelle:</strong> Score &lt;28 (matinal), 28-37
                (intermédiaire), &ge;38 (vespéral)
              </p>
              <p>
                <strong>Flexibilité:</strong> Capacité d'adaptation aux
                changements d'horaires. <strong>Languide:</strong> Tendance à la
                fatigue et au besoin de sommeil.
              </p>
            </div>
          </div>
        )}
        ...
        {code === "ALS18" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="text-xs text-gray-600 mt-2 pt-2 border-t space-y-1">
              <p>
                <strong>Cotation:</strong> 18 items, 0-3 par item (0=Absolument
        ...
        {code === "AIM" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-medium">
                    {data.total_score !== null &&
                      data.total_score >= 81 &&
                      "Intensité affective élevée"}
                    {data.total_score !== null &&
                      data.total_score >= 51 &&
                      data.total_score < 81 &&
                      "Intensité affective normale"}
                    {(data.total_score === null || data.total_score < 51) &&
                      "Intensité affective faible"}
                  </span>
                  <span className="text-xs text-gray-600">
                    Score: {data.total_score ?? "-"}/120
                  </span>
                </div>
                <p className="text-xs leading-relaxed mt-2">
                  {data.total_score !== null &&
            ...
            <div className="flex justify-between items-center pt-2 border-t">
              ...
              <span className="font-semibold">
                {data.total_mean !== null && data.total_mean !== undefined
                  ? parseFloat(data.total_mean).toFixed(2)
                  : "-"}
                /6.0
              </span>
            </div>
            ...
            <div className="text-xs text-gray-600 mt-2 pt-2 border-t space-y-1">
              <p>
                <strong>Cotation:</strong> 20 items, 1-6 par item (1=Jamais,
                6=Toujours). 6 items inversés.
              </p>
              <p>
                <strong>Seuils:</strong> 20-50 (faible) | 51-80 (normale) |
                81-120 (élevée)
              </p>
            </div>
          </div>
        ...
        {code === "AQ12" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="text-xs text-gray-600 mt-2 pt-2 border-t space-y-1">
              <p>
                <strong>Cotation:</strong> 12 items (BPAQ-12), 1-6 par item
                (1=Pas du tout moi, 6=Tout à fait moi)
              </p>
              <p>
                <strong>Seuils:</strong> 12-30 (faible) | 31-45 (modérée) |
                &gt;45 (élevée)
              </p>
              <p className="text-gray-500 italic mt-1">
                En clinique, on s'intéresse à la comparaison interindividuelle,
                à l'évolution dans le temps, et au lien avec l'impulsivité, les
                troubles de l'humeur, et le stress.
              </p>
            </div>
          </div>
        )}
        ...
        {code === "MADRS" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
              <p className="text-blue-900 text-xs leading-relaxed">
                Cette questionnaire évalue la gravité des symptômes dans des
                domaines très variés tels que l'humeur, le sommeil et l'appétit,
                la fatigue physique et psychique et les idées de suicide.
                L'échelle MADRS constitue un bon complément à l'échelle de
        ...
        {(code === "YMRS" || code === "YMRS_SZ") && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
              <p className="text-blue-900 text-xs leading-relaxed">
                C'est une échelle clinique utilisée pour quantifier la sévérité
                des symptômes maniaques. Le score total reflète l'intensité
                globale de la manie. En pratique clinique, un score ≥ 20 est
                généralement considéré comme cliniquement significatif et
                justifie une prise en charge thérapeutique adaptée.
              </p>
            </div>
            ...
            >
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-base">
                    ...
                    {data.total_score !== null &&
                      data.total_score >= 0 &&
                      data.total_score <= 12 &&
                      "Absence de manie / symptômes minimes"}
                  </span>
                  <span className="text-xs text-gray-600 font-medium">
                    Score: {data.total_score ?? "-"}/60
                  </span>
                ...
                <div className="pt-2 border-t">
                  <p className="text-xs leading-relaxed text-gray-700">
                    {data.total_score !== null && data.total_score <= 12 && (
                      <>
                        État euthymique ou traits résiduels non cliniquement
                        significatifs. Fonctionnement global préservé, pas de
                        retentissement fonctionnel notable. Score utilisé comme
                        seuil de rémission.
                      </>
                    )}
                    {data.total_score !== null &&
                      ...
                      data.total_score <= 19 && (
                        <>
                          <strong>Zone grise clinique.</strong> Symptômes
                          présents mais partiellement contrôlés (énergie accrue,
                          réduction du sommeil, discours rapide). Pas ou peu de
                          rupture fonctionnelle, jugement conservé.{" "}
                          <strong className="text-blue-700">
                            Signal d'alerte :
                          </strong>{" "}
                          risque d'escalade vers une manie franche. Ajustement
                          thérapeutique précoce recommandé.
                        </>
                      )}
                    {data.total_score !== null &&
                      data.total_score >= 20 &&
                      data.total_score <= 25 && (
                        <>
                          Symptômes clairement pathologiques : agitation motrice
                          marquée, irritabilité persistante, accélération
                          idéique, début d'altération du jugement.{" "}
                          <strong className="text-amber-700">
                            Retentissement fonctionnel net.
                          </strong>{" "}
                          Risque accru de comportements impulsifs. Prise en
                          charge active nécessaire avec évaluation de la
                          sécurité.
                        </>
                      )}
                    {data.total_score !== null && data.total_score >= 26 && (
                      <>
                        Manie franche, souvent désorganisée : agitation intense,
                        logorrhée, idées de grandeur, possible symptomatologie
                        psychotique.{" "}
                        <strong className="text-red-700">
                          Altération majeure du fonctionnement, risque élevé.
                        </strong>{" "}
                        Indication fréquente d'hospitalisation. Épisode aigu non
                        stabilisé.
                      </>
                    )}
                  </p>
                ...
                {data.total_score !== null && data.total_score >= 20 && (
                  <div className="pt-2 border-t">
                    <p
                      className={`text-xs font-medium ${data.total_score >= 26 ? "text-red-700" : "text-amber-700"}`}
                    >
                      Cliniquement significatif - Prise en charge thérapeutique
                      adaptée {data.total_score >= 26 ? "urgente" : "active"}{" "}
                      recommandée
                    </p>
                  </div>
                )}
            ...
            <div className="text-xs text-gray-500 pt-2 border-t space-y-2">
              <p>
                <strong>Interprétation des seuils :</strong>
              </p>
              <div className="space-y-1 pl-2">
                <p>
                  • <strong>0-12 points :</strong> Absence de manie / symptômes
                  minimes - Seuil de rémission
                </p>
                <p>
                  • <strong>13-19 points :</strong> Hypomanie légère - Zone
                  d'alerte, ajustement thérapeutique précoce
                </p>
                <p>
                  • <strong>20-25 points :</strong> Manie modérée - Prise en
                  charge active, évaluation sécurité
                </p>
                <p>
                  • <strong>≥ 26 points :</strong> Manie sévère - Indication
                  hospitalisation fréquente
                </p>
              </div>
              <p className="mt-2 pt-2 border-t">
                <em>
                  Note: Items 5, 6, 8, 9 sont pondérés x2 (cotés 0-8 au lieu de
                  0-4)
                </em>
              </p>
            </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
              ...
              <div className="flex items-center justify-between relative z-10">
                <div className="flex-1">
                  <span className="text-gray-600 text-sm block mb-1">
                    Niveau de fonctionnement global
                  </span>
            ...
            <div className="mt-3 p-3 bg-white rounded-lg border border-slate-100 shadow-sm">
              <p className="text-xs text-slate-600 italic leading-relaxed">
                L'Échelle Globale de Fonctionnement (EGF) évalue le
                fonctionnement psychologique, social et professionnel du patient
                sur une échelle de 1 à 100. Un score plus élevé indique un
          ...
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            <div className="grid grid-cols-2 gap-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">
                  {data.total_raw_score ?? "-"}/36
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Note Standard:</span>
                <span className="font-semibold">
                  {data.standard_score ?? "-"}/19
                </span>
              </div>
            </div>
            <div className="flex justify-between pt-2 border-t">
          ...
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            <div className="grid grid-cols-2 gap-2">
              <div className="flex justify-between">
                <span className="text-gray-600">Score Brut Total:</span>
                <span className="font-semibold">
                  {data.wais_mat_tot ?? "-"}/26
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Note Standard:</span>
                <span className="font-semibold">
                  {data.wais_mat_std ?? "-"}/19
                </span>
              </div>
            </div>
            <div className="flex justify-between pt-2 border-t">
              <span className="text-gray-600">
                Valeur Standardisée (Z-score):
              </span>
              <span className="font-semibold">
                {data.wais_mat_cr !== null && data.wais_mat_cr !== undefined
                  ? Number(data.wais_mat_cr).toFixed(2)
                  : "-"}
              </span>
            </div>
            <div className="flex justify-between">
            ...
            {data.wais_mat_std === null && data.wais_mat_tot !== null && (
              <div className="mt-2 p-2 bg-yellow-50 rounded text-yellow-800 text-xs">
                Note: Le score standard est nul car la règle d'arrêt a été
                atteinte (4 items consécutifs à 0 ou 4/5 items à 0).
              </div>
            )}
          </div>
        )}
        ...
        {code === "SSTICS_SZ" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="mt-2 p-2 bg-blue-50 rounded text-blue-800 text-xs">
              <strong>Interprétation:</strong> Un Z-score négatif indique plus
              de plaintes cognitives subjectives que la population de référence
              (Moyenne = 13.1, ET = 6.2).
            </div>
          </div>
        )}
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="bg-slate-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className={`text-lg font-bold ${data.cbq_total_z > 1.65 ? "text-red-600" : "text-slate-700"}`}
                >
                  {data.cbq_total ?? "-"}/90
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 text-xs">
                  Z-score (vs. groupe contrôle)
                </span>
                <span
                  className={`font-semibold ${data.cbq_total_z > 1.65 ? "text-red-600" : ""}`}
                >
                  Z ={" "}
                  {data.cbq_total_z !== null && data.cbq_total_z !== undefined
                    ? Number(data.cbq_total_z).toFixed(2)
                    : "-"}
            ...
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">
                Scores par sous-échelle (5 types de biais cognitifs)
              </h5>
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div>
                    ...
                    <span className="text-gray-500 ml-1">
                      (6 items: Q1,3,11,13,16,21)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-semibold">
                      {data.cbq_intentionalisation ?? "-"}/18
                    </span>
                    <span
                      className={`ml-2 ${data.cbq_intentionalisation_z > 1.65 ? "text-red-600 font-bold" : "text-gray-600"}`}
                    >
                      Z:{" "}
                ...
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div>
                    ...
                    <span className="text-gray-500 ml-1">
                      (5 items: Q2,4,12,14,22)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-semibold">
                      {data.cbq_catastrophisation ?? "-"}/15
                    </span>
                    <span
                      className={`ml-2 ${data.cbq_catastrophisation_z > 1.65 ? "text-red-600 font-bold" : "text-gray-600"}`}
                    >
                      Z:{" "}
                ...
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div>
                    ...
                    <span className="text-gray-500 ml-1">
                      (6 items: Q5,8,18,23,26,28)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-semibold">
                      {data.cbq_pensee_dichotomique ?? "-"}/18
                    </span>
                    <span
                      className={`ml-2 ${data.cbq_pensee_dichotomique_z > 1.65 ? "text-red-600 font-bold" : "text-gray-600"}`}
                    >
                      Z:{" "}
                ...
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div>
                    <span className="text-gray-700 font-medium">
                      Sauter aux conclusions (JTC)
                    </span>
                    <span className="text-gray-500 ml-1">
                      (6 items: Q6,10,15,20,24,30)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-semibold">
                      {data.cbq_sauter_conclusions ?? "-"}/18
                    </span>
                    <span
                      className={`ml-2 ${data.cbq_sauter_conclusions_z > 1.65 ? "text-red-600 font-bold" : "text-gray-600"}`}
                    >
                      Z:{" "}
                ...
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  <div>
                    ...
                    <span className="text-gray-500 ml-1">
                      (7 items: Q7,9,17,19,25,27,29)
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-semibold">
                      {data.cbq_raisonnement_emotionnel ?? "-"}/21
                    </span>
                    <span
                      className={`ml-2 ${data.cbq_raisonnement_emotionnel_z > 1.65 ? "text-red-600 font-bold" : "text-gray-600"}`}
                    >
                      Z:{" "}
            ...
            <div className="pt-2 border-t">
              <h5 className="font-semibold text-gray-700 mb-2">
                Dimensions thématiques (2 thèmes)
              </h5>
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-2 bg-gray-50 rounded">
                  <div className="text-gray-700 font-medium">
                    Thème 1: Événement Menaçant
                  </div>
                  <div className="text-gray-500 text-xs mb-1">
                    Items Q1-Q15 (15 items)
                  </div>
                  <div className="font-semibold text-base">
                    {data.cbq_evenement_menacant ?? "-"}/45
                  </div>
                </div>
                <div className="p-2 bg-gray-50 rounded">
                  <div className="text-gray-700 font-medium">
                    Thème 2: Perception Anormale
                  </div>
                  <div className="text-gray-500 text-xs mb-1">
                    Items Q16-Q30 (15 items)
                  </div>
                  <div className="font-semibold text-base">
                    {data.cbq_perception_anormale ?? "-"}/45
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded text-blue-800 text-xs">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <strong>Score par item:</strong> 1 = absence de biais, 2 =
                  biais possible, 3 = biais manifeste
                </li>
                <li>
                  <strong>Calcul du Z-score:</strong> Z = (Score brut - Moyenne
                  contrôle) / Écart-type contrôle
                </li>
                <li>
                  <strong>Seuil clinique:</strong> Z &gt; +1.65 indique des
                  biais cognitifs cliniquement significatifs
                </li>
                <li>
                  <strong>Normes contrôles:</strong> Moyenne totale = 36.5, ET =
                  2.7
                </li>
              </ul>
            </div>
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="bg-slate-50 p-3 rounded-lg">
              ...
              <div className="text-xs text-gray-600">
                Score de 42 (minimum) à 294 (maximum). Plus le score est élevé,
                plus les biais cognitifs et comportements de sûreté sont
                présents.
              </div>
            </div>
            ...
            <div className="bg-amber-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-amber-800">
                  Section 2: Limitations cognitives
                </span>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  <div>
                    <span className="text-gray-700 font-medium">
                      Problèmes sociaux cognitifs (SC)
                    </span>
                    <span className="text-gray-500 ml-1">
                      (Q4,9,11,14,19,39)
                    </span>
                ...
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  <div>
                    <span className="text-gray-700 font-medium">
                      Problèmes cognitifs subjectifs (CP)
                    </span>
                    <span className="text-gray-500 ml-1">
                      (Q5,21,28,32,36,40)
                    </span>
            ...
            <div className="bg-purple-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-purple-800">
                  Section 3: Comportements de sûreté
                </span>
            ...
            <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded text-blue-800 text-xs">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <strong>Échelle:</strong> 1 (Fortement en désaccord) à 7
                  (Fortement d&apos;accord)
                </li>
                <li>
                  <strong>Chaque sous-échelle:</strong> 6 items × 7 points max =
                  42 points
                </li>
                <li>
                  <strong>Section 1 (Biais cognitifs):</strong> 4 sous-échelles
                  × 42 = 168 points max
                </li>
                <li>
                  <strong>Section 2 (Limitations cognitives):</strong> 2
                  sous-échelles × 42 = 84 points max
                </li>
                <li>
                  <strong>Section 3 (Comportements de sûreté):</strong> 1
                  sous-échelle × 42 = 42 points max
                </li>
                <li>
                  <strong>Score total:</strong> Somme des 42 items (range
                  42-294)
                </li>
              </ul>
            </div>
        ...
        {code === "BRIEF_A_SZ" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="bg-slate-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-600">
                    {formatBriefNorm(data.brief_a_gec_t, data.brief_a_gec_p)}
                  </div>
                </div>
              </div>
              <div className="text-xs text-gray-600">
                Score de 75 (minimum) à 225 (maximum). Plus le score est élevé,
                plus les difficultés exécutives sont importantes.
              </div>
            </div>
            ...
            <div className="bg-blue-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-blue-800">
                  Indice de Régulation Comportementale (BRI)
                </span>
                <div className="text-right">
                  ...
                  <div className="text-xs text-blue-700">
                    {formatBriefNorm(data.brief_a_bri_t, data.brief_a_bri_p)}
                  </div>
                </div>
              </div>
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_inhibit ?? "-"}/24
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_inhibit_t,
                        data.brief_a_inhibit_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_shift ?? "-"}/18
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_shift_t,
                        data.brief_a_shift_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_emotional_control ?? "-"}/30
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_emotional_control_t,
                        data.brief_a_emotional_control_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_self_monitor ?? "-"}/18
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_self_monitor_t,
                        data.brief_a_self_monitor_p,
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-amber-700">
                    {formatBriefNorm(data.brief_a_mi_t, data.brief_a_mi_p)}
                  </div>
                </div>
              </div>
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_initiate ?? "-"}/24
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_initiate_t,
                        data.brief_a_initiate_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_working_memory ?? "-"}/24
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_working_memory_t,
                        data.brief_a_working_memory_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_plan_organize ?? "-"}/30
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_plan_organize_t,
                        data.brief_a_plan_organize_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_task_monitor ?? "-"}/18
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_task_monitor_t,
                        data.brief_a_task_monitor_p,
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">
                      {data.brief_a_organization_materials ?? "-"}/24
                    </div>
                    <div className="text-[11px] text-gray-500">
                      {formatBriefNorm(
                        data.brief_a_organization_materials_t,
                        data.brief_a_organization_materials_p,
                      )}
                    </div>
                  </div>
                </div>
            ...
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <span
                    className={`font-semibold ${(data.brief_a_negativity ?? 0) >= 6 ? "text-red-600" : ""}`}
                  >
                ...
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <span
                    className={`font-semibold ${(data.brief_a_inconsistency ?? 0) >= 8 ? "text-red-600" : ""}`}
                  >
                ...
                <div className="flex justify-between items-center p-2 bg-white rounded">
                  ...
                  <span
                    className={`font-semibold ${(data.brief_a_infrequency ?? 0) >= 3 ? "text-orange-600" : ""}`}
                  >
            ...
            <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded text-blue-800 text-xs">
              ...
              <ul className="list-disc list-inside space-y-1">
                <li>
                  <strong>Échelle:</strong> 1 (Jamais) à 3 (Souvent)
                </li>
                <li>
                  <strong>BRI:</strong> Inhibition + Flexibilité + Contrôle
                  émotionnel + Auto-contrôle
                </li>
                <li>
                  <strong>MI:</strong> Initiation + Mémoire de travail +
                  Planification + Contrôle de la tâche + Organisation du
                  matériel
                </li>
                <li>
                  <strong>GEC:</strong> BRI + MI (score total des 75 items)
                </li>
                <li>
                  <strong>Interprétation:</strong> Scores plus élevés = plus de
        ...
        {/* BRIEF-A Auto Details */}
        {code === 'BRIEF_A_AUTO_SZ' && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            {/* Global Executive Composite Summary */}
            <div className="bg-slate-50 p-3 rounded-lg">
              <div className="flex justify-between items-center mb-2">
                <span className="font-semibold text-gray-700">Score Exécutif Global (GEC) - Auto-évaluation</span>
                <span className="text-lg font-bold text-slate-700">
                  {data.brief_a_gec ?? '-'}/225
                </span>
              </div>
            ...
            {/* Validity Scales - only Negativity and Infrequency for auto version */}
            <div className="bg-gray-50 p-3 rounded-lg">
              ...
              <div className="mt-2 text-xs text-gray-500">
                Note: L'échelle d'Incohérence n'est pas calculée pour l'auto-évaluation
              </div>
            </div>
            ...
            <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded text-blue-800 text-xs">
              ...
              <ul className="list-disc list-inside space-y-1">
                ...
                <li><strong>Interprétation:</strong> Scores plus élevés = plus de difficultés exécutives auto-rapportées</li>
              </ul>
            </div>
          </div>
        )}
        ...
        {code === "WAIS4_MATRICES" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">
                  Note standard à la WAIS MATRICE:
                </span>
                <span className="font-semibold">
                  {data.standardized_score ?? "-"}/19
                </span>
            ...
            <div className="flex justify-between pt-2 border-t">
              <span className="text-gray-600">
                Déviation par rapport à la moyenne et l'écart type:
              </span>
              <span className="font-semibold">
          ...
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="grid grid-cols-3 gap-2 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {data.patient_sex === "F" ? "Femme" : "Homme"}
                </span>
              </div>
            </div>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {data.tmta_tps_z ?? "-"} / {data.tmta_tps_pc ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Z-Score / Percentile:</span>
                  <span className="font-medium">
                    {data.tmta_errtot_z ?? "-"} / {data.tmta_errtot_pc ?? "-"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Z-Score / Percentile:</span>
                  <span className="font-medium">
                    {data.tmtb_tps_z ?? "-"} / {data.tmtb_tps_pc ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Erreurs totales:</span>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Z-Score / Percentile:</span>
                  <span className="font-medium">
                    {data.tmtb_errtot_z ?? "-"} / {data.tmtb_errtot_pc ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Erreurs persév:</span>
                  <span className="font-medium">
                    {data.tmtb_err_persev ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Z-Score / Percentile:</span>
                  <span className="font-medium">
                    {data.tmtb_err_persev_z ?? "-"} /{" "}
                    {data.tmtb_err_persev_pc ?? "-"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">Z-Score / Percentile:</span>
                  <span className="font-medium">
                    {data.tmt_b_a_tps_z ?? "-"} / {data.tmt_b_a_tps_pc ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            >
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-base">
                    {data.lis_score === null || data.lis_score === undefined
                      ? "Questionnaire incomplet"
                      : data.lis_score === 0
                        ? "Parfait - Correspondance exacte"
                        : data.lis_score <= 10
                          ? "Excellente cognition sociale"
                          : data.lis_score <= 20
                            ? "Bonne cognition sociale"
                            : data.lis_score <= 30
                              ? "Cognition sociale modérée"
                              : data.lis_score <= 45
                                ? "Difficultés de cognition sociale"
                                : "Difficultés importantes de cognition sociale"}
                  </span>
                </div>
                <p className="text-xs text-gray-600">
                  Score de déviation = Somme des écarts absolus par rapport aux
                  valeurs normatives. Plus le score est bas, meilleure est la
                  cognition sociale.
                </p>
              </div>
            </div>
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              <div className="flex justify-between items-center">
                <span className="font-semibold text-gray-700">
                  Score total de déviation
                </span>
                <span className="text-xl font-bold text-gray-900">
                  {data.lis_score !== null && data.lis_score !== undefined
                    ? data.lis_score.toFixed(2)
                    : "-"}
                </span>
              </div>
          ...
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            >
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="font-semibold text-base">
                    {data.qi_interpretation ||
                      (data.qi_indice === null || data.qi_indice === undefined
                        ? "Non calculé"
                        : data.qi_indice >= 130
                          ? "Très supérieur"
                          : data.qi_indice >= 120
                ...
                <p className="text-xs text-gray-600">
                  Estimation du QI selon Denney 2015 à partir des notes standard
                  des 7 subtests WAIS-IV
                </p>
              </div>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-gray-900 font-semibold">
                      {data.qi_indice ?? "-"} (
                      {data.qi_rang ? `P${data.qi_rang}` : "-"})
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    Somme: {data.qi_sum_std ?? "-"} | IC95%:{" "}
                    {data.qi_ci95 ?? "-"}
                  </div>
                </div>
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-gray-900 font-semibold">
                      {data.icv_indice ?? "-"} (
                      {data.icv_rang ? `P${data.icv_rang}` : "-"})
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    Somme: {data.icv_sum_std ?? "-"} | IC95%:{" "}
                    {data.icv_ci95 ?? "-"}
                  </div>
                </div>
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-gray-900 font-semibold">
                      {data.irp_indice ?? "-"} (
                      {data.irp_rang ? `P${data.irp_rang}` : "-"})
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    Somme: {data.irp_sum_std ?? "-"} | IC95%:{" "}
                    {data.irp_ci95 ?? "-"}
                  </div>
                </div>
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-gray-900 font-semibold">
                      {data.imt_indice ?? "-"} (
                      {data.imt_rang ? `P${data.imt_rang}` : "-"})
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    Somme: {data.imt_sum_std ?? "-"} | IC95%:{" "}
                    {data.imt_ci95 ?? "-"}
                  </div>
                </div>
                ...
                <div className="p-2 bg-gray-50 rounded">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="text-gray-900 font-semibold">
                      {data.ivt_indice ?? "-"} (
                      {data.ivt_rang ? `P${data.ivt_rang}` : "-"})
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    Somme: {data.ivt_sum_std ?? "-"} | IC95%:{" "}
                    {data.ivt_ci95 ?? "-"}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  <div className="p-2 bg-gray-50 rounded">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-gray-900 font-semibold">
                        {data.barona_qit_attendu != null
                          ? Number(data.barona_qit_attendu).toFixed(1)
                          : "-"}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                      Estimation du QI prémorbide basée sur les facteurs
                      démographiques
                    </p>
                  </div>
                  ...
                  >
                    ...
                    {data.barona_qit_difference != null &&
                      Number(data.barona_qit_difference) >= 19 && (
                        <p className="text-xs text-red-600 mt-1 font-medium">
                          Différence significative (p &lt; 0.05) - Possible
        ...
        {code === "WAIS4_CODE" && (
          <div className="text-sm space-y-2 mt-2 pt-2 border-t">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-lg">
                {data.wais_cod_cr !== null && data.wais_cod_cr !== undefined
                  ? data.wais_cod_cr
                  : "-"}
              </span>
            </div>
        ...
        {code === "WAIS_IV_CODE_SYMBOLES_IVT" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            {data.wais_symb_tot !== null &&
              data.wais_symb_tot !== undefined && (
                <div className="pt-2 border-t">
                  <h5 className="font-semibold text-gray-700 mb-2">
                    Subtest Symboles
                  </h5>
        ...
        {code === "FLUENCES_VERBALES" && (
          <div className="text-sm space-y-3 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              <h5 className="font-semibold text-gray-700 mb-2">
                Fluence Sémantique - Animaux
              </h5>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between">
                  <span className="text-gray-600">Mots corrects:</span>
                  <span className="font-semibold">
                    {data.fv_anim_tot_correct ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Ruptures de règle:</span>
                  <span className="font-medium">
                    {data.fv_anim_tot_rupregle ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Z-Score:</span>
        ...
        {code === "PANSS" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">
                Scores canoniques PANSS
              </h5>
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">
                    Symptomes positifs (P1-P7):
                  </span>
                  <span className="font-semibold">
                    {data.positive_score ?? "-"}/49
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">
                    Symptomes negatifs (N1-N7):
                  </span>
                  <span className="font-semibold">
                    {data.negative_score ?? "-"}/49
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">
                    Psychopathologie generale (G1-G16):
                  </span>
                  <span className="font-semibold">
                    {data.general_score ?? "-"}/112
                  </span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t">
                  <span className="text-gray-600 font-medium">
            ...
            <div className="pt-2 border-t">
              <h5 className="font-semibold text-gray-700 mb-2">
                Modele a 5 facteurs - Lancon 1998
              </h5>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                <div className="flex justify-between">
                  <span className="text-gray-500">Positif:</span>
                  <span className="font-medium">
                    {data.lancon_positive ?? "-"}/35
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Negatif:</span>
                  <span className="font-medium">
                    {data.lancon_negative ?? "-"}/49
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Desorganise:</span>
                  <span className="font-medium">
                    {data.lancon_disorganized ?? "-"}/21
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Excite:</span>
                  <span className="font-medium">
                    {data.lancon_excited ?? "-"}/35
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Depressif:</span>
                  <span className="font-medium">
                    {data.lancon_depressed ?? "-"}/28
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="pt-2 border-t">
              <h5 className="font-semibold text-gray-700 mb-2">
                Modele a 5 facteurs - Van der Gaag 2006
              </h5>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                <div className="flex justify-between">
                  <span className="text-gray-500">Positif:</span>
                  <span className="font-medium">
                    {data.vandergaag_positive ?? "-"}/35
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Negatif:</span>
                  <span className="font-medium">
                    {data.vandergaag_negative ?? "-"}/56
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Desorganise:</span>
                  <span className="font-medium">
                    {data.vandergaag_disorganized ?? "-"}/56
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Excite:</span>
                  <span className="font-medium">
                    {data.vandergaag_excited ?? "-"}/28
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Depressif:</span>
                  <span className="font-medium">
                    {data.vandergaag_depressed ?? "-"}/28
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    1. La vie ne vaut pas la peine:
                  </span>
                  <span className="font-medium">
                    {data.q1_life_worth === 1
                      ? "Oui"
                      : data.q1_life_worth === 0
                        ? "Non"
                        : "-"}
                  </span>
                </div>
                {data.q1_life_worth === 1 && data.q1_time && (
                  <div className="flex justify-between ml-4 text-gray-500">
                    ...
                    <span className="text-xs">
                      {data.q1_time === "last_week"
                        ? "Semaine derniere"
                        : "Depuis derniere visite"}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {data.q2_wish_death === 1
                      ? "Oui"
                      : data.q2_wish_death === 0
                        ? "Non"
                        : "-"}
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    4. Plan ou consideration serieuse:
                  </span>
                  <span className="font-medium">
                    {data.q4_plan === 1
                      ? "Oui"
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Doses prescrites par jour:
                  </span>
                  <span className="font-medium">{data.q1 ?? "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Jours sans traitement (30 derniers jours):
                  </span>
                  <span className="font-medium">{data.q2 ?? "-"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Jours avec dose reduite (30 derniers jours):
                  </span>
            ...
            <div className="flex flex-col gap-2 pt-2 border-t">
              <div className="flex justify-between items-center">
                <span className="text-gray-600 font-medium">
                  Calcul de l'observance (formule BARS):
                </span>
                <span className="font-bold text-lg">
              ...
              <div className="flex justify-between items-center">
                <span className="text-gray-600 font-medium">
                  Estimation manuelle de l'observance:
                </span>
                <span className="font-bold text-lg text-brand">
                  {data.estimation_observance ?? "-"}%
                </span>
              </div>
            </div>
          </div>
        ...
        {code === "SUMD" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">
                Conscience globale (Items 1-3)
              </h5>
              <div className="grid grid-cols-1 gap-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">1. Trouble mental:</span>
                  <span
                    className={`font-medium ${data.conscience1 === 3 ? "text-amber-600" : data.conscience1 === 1 ? "text-green-600" : ""}`}
                  >
                    {data.conscience1 === 0
                      ? "Non cotable"
                      : data.conscience1 === 1
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    2. Consequences du trouble:
                  </span>
                  <span
                    className={`font-medium ${data.conscience2 === 3 ? "text-amber-600" : data.conscience2 === 1 ? "text-green-600" : ""}`}
                  >
                    {data.conscience2 === 0
                      ? "Non cotable"
                      : data.conscience2 === 1
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    3. Effets du traitement:
                  </span>
                  <span
                    className={`font-medium ${data.conscience3 === 3 ? "text-amber-600" : data.conscience3 === 1 ? "text-green-600" : ""}`}
                  >
                    {data.conscience3 === 0
                      ? "Non cotable"
                      : data.conscience3 === 1
            ...
            <div>
              <h5 className="font-semibold text-gray-700 mb-2">
                Conscience des symptomes (Items 4-9)
              </h5>
              <div className="space-y-3">
                {/* Item 4: Hallucinations */}
                <div className="border-l-2 border-gray-200 pl-3">
                  <div className="font-medium text-gray-700 mb-1">
                    4. Experience hallucinatoire
                  </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                NC = Non cotable | Conscience: 1=Conscient, 2=Partiel,
                3=Inconscient
              </p>
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q1 !== null && data.q1 >= 3 ? "text-amber-600" : data.q1 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q1 ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q2 !== null && data.q2 >= 3 ? "text-amber-600" : data.q2 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q2 ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q3 !== null && data.q3 >= 3 ? "text-amber-600" : data.q3 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q3 ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q4 !== null && data.q4 >= 3 ? "text-amber-600" : data.q4 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q4 ?? "-"}
                  </span>
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q5 !== null && data.q5 >= 3 ? "text-amber-600" : data.q5 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q5 ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q6 !== null && data.q6 >= 3 ? "text-amber-600" : data.q6 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q6 ?? "-"}
                  </span>
                </div>
                <div className="flex justify-between col-span-2">
                  ...
                  <span
                    className={`font-medium ${data.q7 !== null && data.q7 >= 3 ? "text-amber-600" : data.q7 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q7 ?? "-"}
                  </span>
                </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q8 !== null && data.q8 >= 3 ? "text-amber-600" : data.q8 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q8 === 0
                      ? "Aucun"
                      : data.q8 === 1
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q9 !== null && data.q9 >= 3 ? "text-amber-600" : data.q9 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q9 === 0
                      ? "Aucune"
                      : data.q9 === 1
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    11. Problemes dentaires:
                  </span>
                  <span className="font-medium">
                    {data.q11 === 0 ? "Oui" : data.q11 === 1 ? "Non" : "-"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">12. Porte prothese:</span>
                  <span className="font-medium">
                    {data.q12 === 0 ? "Oui" : data.q12 === 1 ? "Non" : "-"}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between pt-2 border-t">
              <span className="text-gray-600 font-medium">
                Score de mouvement (items 1-7):
              </span>
              <span className="font-bold text-lg">
                {data.movement_score ?? "-"}/28
              </span>
            </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                Echelle de severite: 0=Aucun, 1=Minime, 2=Leger, 3=Moyen,
                4=Grave
              </p>
            </div>
          </div>
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Score obj+subj (items 1-3):
                  </span>
                  <span className="font-bold">
                    {data.objective_subjective_score ?? "-"}/9
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                Score global: 0=Absence, 1=Douteux, 2=Legere, 3=Moyenne,
                4=Marquee, 5=Severe
              </p>
              <p className="mt-1">
                Note: Mouvements sans agitation subjective = pseudo-akathisie
                (score 0)
              </p>
            </div>
          </div>
        ...
        {code === "SAS" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q1 >= 2 ? "text-amber-600" : data.q1 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q1 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q2 >= 2 ? "text-amber-600" : data.q2 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q2 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q3 >= 2 ? "text-amber-600" : data.q3 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q3 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q4 >= 2 ? "text-amber-600" : data.q4 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q4 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q5 >= 2 ? "text-amber-600" : data.q5 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q5 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q6 >= 2 ? "text-amber-600" : data.q6 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q6 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q7 >= 2 ? "text-amber-600" : data.q7 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q7 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q8 >= 2 ? "text-amber-600" : data.q8 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q8 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q9 >= 2 ? "text-amber-600" : data.q9 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q9 ?? "-"}/4
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-medium ${data.q10 >= 2 ? "text-amber-600" : data.q10 === 0 ? "text-green-600" : ""}`}
                  >
                    {data.q10 ?? "-"}/4
                  </span>
                </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                Score: 0 = Normal, 1 = Leger, 2 = Modere, 3 = Important, 4 =
                Severe
              </p>
              <p className="mt-1">
                Seuil clinique: Score moyen &gt; 0.3 = EPS significatifs
              </p>
            </div>
          </div>
        )}
        ...
        {code === "PSP" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {data.interval_selection
                      ? data.interval_selection === 1
                        ? "91-100"
                        : data.interval_selection === 2
                          ? "81-90"
                          : data.interval_selection === 3
                            ? "71-80"
                            : data.interval_selection === 4
                              ? "61-70"
                              : data.interval_selection === 5
                                ? "51-60"
                                : data.interval_selection === 6
                                  ? "41-50"
                                  : data.interval_selection === 7
                                    ? "31-40"
                                    : data.interval_selection === 8
                                      ? "21-30"
                                      : data.interval_selection === 9
                                        ? "11-20"
                                        : "1-10"
                      : "-"}
                  </span>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                Score: 91-100 = Tres bon, 71-90 = Bon, 51-70 = Modere, 31-50 =
                Altere, 1-30 = Severement altere
              </p>
              <p className="mt-1">
                Periode d'evaluation: Le mois dernier (30 derniers jours)
              </p>
            </div>
          </div>
        )}
        ...
        {(code === "EQ5D5L" || code === "EQ5D5L_SZ") && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
                  ...
                  >
                    {data.usual_activities === 1
                      ...
                      : data.usual_activities === 2
                        ...
                        : data.usual_activities === 3
                          ? "Problèmes modérés"
                          : data.usual_activities === 4
                            ? "Problèmes sévères"
                            : data.usual_activities === 5
                              ? "Incapacité"
                              : "-"}
                  </span>
                </div>
                  ...
                  >
                    {data.pain_discomfort === 1
                      ...
                      : data.pain_discomfort === 2
                        ...
                        : data.pain_discomfort === 3
                          ? "Modérée"
                          : data.pain_discomfort === 4
                            ? "Sévère"
                            : data.pain_discomfort === 5
                              ? "Extrême"
                              : "-"}
                  </span>
                </div>
                  ...
                  >
                    {data.anxiety_depression === 1
                      ...
                      : data.anxiety_depression === 2
                        ...
                        : data.anxiety_depression === 3
                          ? "Modérée"
                          : data.anxiety_depression === 4
                            ? "Sévère"
                            : data.anxiety_depression === 5
                              ? "Extrême"
                              : "-"}
                  </span>
                </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>Profil:</strong> Codage 5 dimensions (1=aucun problème,
                5=pire état)
              </p>
              <p className="mt-1">
                <strong>VAS:</strong> Échelle visuelle analogique de l'état de
                santé (0=pire, 100=meilleur)
              </p>
              <p className="mt-1">
                <strong>Index:</strong> Valeur d'utilité de qualité de vie
                (1.0=santé parfaite, &lt;0=pire que la mort)
              </p>
            </div>
          </div>
        )}
        ...
        {code === "IPAQ_SZ" && (
          <div className="text-sm space-y-4 mt-4 pt-4 border-t-2 border-gray-200">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="text-center p-3 bg-red-50 rounded-lg border border-red-200">
                  ...
                  <div className="text-xs font-medium text-red-800 mt-1">
                    Activité intense
                  </div>
                  <div className="text-xs text-gray-500">MET-min/sem</div>
                  {data.vigorous_days > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      {data.vigorous_days}j ×{" "}
                      {(data.vigorous_hours || 0) * 60 +
                        (data.vigorous_minutes || 0)}
                      min
                    </div>
                  )}
                </div>
                <div className="text-center p-3 bg-orange-50 rounded-lg border border-orange-200">
                  ...
                  {data.moderate_days > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      {data.moderate_days}j ×{" "}
                      {(data.moderate_hours || 0) * 60 +
                        (data.moderate_minutes || 0)}
                      min
                    </div>
                  )}
                </div>
                <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  {data.walking_days > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      {data.walking_days}j ×{" "}
                      {(data.walking_hours || 0) * 60 +
                        (data.walking_minutes || 0)}
                      min
                    </div>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  ...
                  <span className="font-medium text-gray-900">
                    {data.sitting_weekday_total !== null
                      ? `${Math.floor(data.sitting_weekday_total / 60)}h${data.sitting_weekday_total % 60 > 0 ? ` ${data.sitting_weekday_total % 60}min` : ""}`
                      : "-"}
                  </span>
                </div>
                <div className="flex justify-between items-center p-2 bg-gray-50 rounded">
                  ...
                  <span className="font-medium text-gray-900">
                    {data.sitting_weekend_total !== null
                      ? `${Math.floor(data.sitting_weekend_total / 60)}h${data.sitting_weekend_total % 60 > 0 ? ` ${data.sitting_weekend_total % 60}min` : ""}`
                      : "-"}
                  </span>
                </div>
              </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>IPAQ:</strong> International Physical Activity
                Questionnaire - Version courte (7 derniers jours)
              </p>
              <p className="mt-1">
                <strong>MET:</strong> Équivalent métabolique (Intense=8.0,
                Modéré=4.0, Marche=3.3)
              </p>
              <p className="mt-1">
                <strong>Classification:</strong> ÉLEVÉ (≥1500 MET intense ou
                ≥3000 total) | MODÉRÉ (≥600 MET) | FAIBLE (&lt;600 MET)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>OMS:</strong> ≥150 min/semaine d'activité modérée ou ≥75
                min/semaine d'activité intense recommandés.
              </p>
            </div>
          </div>
        )}
        ...
        {code === "PRISE_M" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>Cotation:</strong> 31 items, 0-2 par item (0=Absent,
                1=Tolérable, 2=Pénible)
              </p>
              <p className="mt-1">
                <strong>Score total:</strong> Somme des 31 items (0-62)
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> 0 (aucun) | 1-10 (léger) | 11-20
                (modéré) | 21-40 (important) | &gt;40 (sévère)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Un score élevé ou de nombreux effets
                pénibles peuvent justifier un ajustement thérapeutique.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="grid grid-cols-2 gap-4 pt-2">
              ...
              {data.note_t && (
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-700">
                    {parseFloat(data.note_t).toFixed(0)}
                  </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>STAI-YA (Form Y-A):</strong> Inventaire d'Anxiété État -
                Mesure l'anxiété ressentie au moment présent
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 20 items, 1-4 par item. Items
                inversés: 1, 2, 5, 8, 10, 11, 15, 16, 19, 20
              </p>
              <p className="mt-1">
                <strong>Score total:</strong> 20-80 (plus élevé = plus
                d'anxiété)
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> ≤35 (légère) | 36-45 (modérée) | 46-55
                (moyenne-haute) | 56-65 (élevée) | &gt;65 (très élevée)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Le STAI-YA évalue l'anxiété état
                (situationnelle), à distinguer de l'anxiété trait
                (dispositionelle, STAI-YB).
              </p>
            </div>
          </div>
        )}
        ...
        {(code === "MARS" || code === "MARS_SZ") && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            {data.adherence_percentage !== null &&
              data.adherence_percentage !== undefined && (
                <div className="pt-2 border-t">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-600">Taux d'observance:</span>
                    <span className="font-medium text-lg">
                      {parseFloat(data.adherence_percentage).toFixed(0)}%
                    </span>
                  </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>MARS:</strong> Medication Adherence Rating Scale -
                Évalue l'observance et les attitudes face au traitement
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 10 items, 0-1 par item. Items
                positifs (7, 8): Oui=1pt. Items négatifs (1-6, 9, 10): Non=1pt
              </p>
              <p className="mt-1">
                <strong>Score total:</strong> 0-10 (plus élevé = meilleure
                observance)
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> ≥8 (bonne) | 6-7 (modérée) | 4-5
                (faible) | &lt;4 (très faible)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Une faible observance est un facteur de
                risque majeur de rechute. L'identification des obstacles est
                essentielle.
              </p>
            </div>
          </div>
        ...
        {code === "BIS_SZ" && (
          <div className="text-sm space-y-4 mt-4 pt-4 border-t-2 border-gray-200">
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="text-center p-3 bg-green-50 rounded-lg border border-green-200">
                  ...
                  <div className="text-xs font-medium text-green-800 mt-1">
                    Besoin traitement
                  </div>
                  <div className="text-xs text-gray-500">/4</div>
                </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>BIS:</strong> Échelle d'Insight de Birchwood - Évalue la
                conscience de la maladie chez les patients psychotiques
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 8 items ternaires. Items positifs
                (1,4,5,7,8): D'accord=2. Items négatifs (2,3,6): Pas d'accord=2.
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> ≥10 (très bon insight) | 7-9 (bon) |
                4-6 (modéré) | &lt;4 (pauvre)
              </p>
            </div>
          </div>
        ...
        {code === "YBOCS_SZ" && (
          <div className="text-sm space-y-4 mt-4 pt-4 border-t-2 border-gray-200">
            ...
            <div>
              <h5 className="font-semibold text-gray-700 mb-3">
                Scores Y-BOCS - Obsessions-Compulsions
              </h5>
              <div className="grid grid-cols-3 gap-3">
                {/* Total Score */}
                <div className="text-center p-3 bg-gray-100 rounded-lg border-2 border-gray-300">
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>Y-BOCS:</strong> Échelle de Yale-Brown pour les
                Obsessions-Compulsions - Évalue la sévérité du TOC
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 10 items (0-4). Sous-total obsessions
                (Q1-5): 0-20. Sous-total compulsions (Q6-10): 0-20.
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> 0-7 (sous-clinique) | 8-15 (léger) |
                16-23 (modéré) | 24-31 (sévère) | 32-40 (extrême)
              </p>
              <p className="mt-1">
                <strong>Seuil clinique:</strong> ≥16 généralement considéré
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="pt-2 border-t">
              ...
              <div className="grid grid-cols-5 gap-3">
                ...
                <div className="text-center p-2 bg-gray-50 rounded">
                  <div className="text-lg font-bold text-gray-900">
                    {data.motivation !== null && data.motivation !== undefined
                      ? parseFloat(data.motivation).toFixed(2)
                      : "-"}
                  </div>
                  <div className="text-xs text-gray-600 mt-1">Motivation</div>
                </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>MAThyS:</strong> Multidimensional Assessment of Thymic
                States - Évaluation multidimensionnelle des états thymiques
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 20 items, 0-10 par item avec
                demi-points. Items inversés: 5, 6, 7, 8, 9, 10, 17, 18
              </p>
              <p className="mt-1">
                <strong>Score total:</strong> 0-200 (~100 = état euthymique)
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> &lt;60 (dépression marquée) | 60-79
                (tendance dépressive) | 80-120 (euthymie) | 121-140 (tendance
                hypomaniaque) | &gt;140 (hypomanie/manie)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Outil sensible pour détecter les
                variations thymiques subtiles. Utile en suivi longitudinal.
              </p>
            </div>
          </div>
        )}
        ...
        {code === "QIDS_SR16" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-bold text-red-900">
                      Alerte: Idéation suicidaire présente
                    </p>
                    <p className="text-sm text-red-800 mt-1">
                      Le patient rapporte des pensées de mort ou de suicide
                      (Item 12: score {data.q12}). Évaluation clinique urgente
                      du risque suicidaire recommandée.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>QIDS-SR16:</strong> Quick Inventory of Depressive
                Symptomatology - Auto-questionnaire
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 16 items avec scoring par domaines
                (max par domaine). Score total: 0-27
              </p>
              <p className="mt-1">
                <strong>Domaines:</strong> Sommeil (items 1-4, max),
                Appétit/Poids (6-9, max), Psychomoteur (15-16, max), autres
                items scorés directement
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> 0-5 (absence) | 6-10 (légère) | 11-15
                (modérée) | 16-20 (sévère) | 21-27 (très sévère)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Outil validé pour le dépistage et le
                suivi de la dépression. Sensible au changement thérapeutique.
              </p>
            </div>
          </div>
        )}
        ...
        {code === "PSQI" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="pt-2">
              <h5 className="font-semibold text-gray-700 mb-3">
                Scores par composante (0-3 chacun)
              </h5>
            ...
            {data.sleep_efficiency_pct !== null &&
              data.sleep_efficiency_pct !== undefined && (
                <div className="pt-2 border-t">
                  <h5 className="font-semibold text-gray-700 mb-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-xs text-gray-600">
                      {data.time_in_bed_hours !== null &&
                        data.time_in_bed_hours !== undefined && (
                          <span>
                            Temps au lit: {data.time_in_bed_hours.toFixed(1)}h
                          </span>
                        )}
                    </div>
                  </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              ...
              <p className="mt-1">
                <strong>Cotation:</strong> 7 composantes (0-3 chacune). Score
                total: 0-21
              </p>
              <p className="mt-1">
                <strong>Composantes:</strong> Qualité subjective, Latence,
                Durée, Efficience, Perturbations, Médication, Dysfonctionnement
                diurne
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> 0-5 (bonne) | 6-10 (altérée) | 11-15
                (mauvaise) | 16-21 (très mauvaise)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Score &gt; 5 indique une mauvaise qualité
                de sommeil cliniquement significative (Buysse et al., 1989)
              </p>
            </div>
          </div>
        )}
        ...
        {code === "EPWORTH" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            {data.total_score >= 16 && (
              <div className="p-3 bg-red-50 border-2 border-red-300 rounded-lg">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-bold text-red-900">
                      Alerte: Somnolence diurne sévère
                    </p>
                    <p className="text-sm text-red-800 mt-1">
                      Score ≥ 16 : Contre-indication formelle à la conduite
                      automobile. Bilan du sommeil en urgence recommandé
                      (polysomnographie, recherche d'apnée du sommeil,
                      narcolepsie). Orientation rapide vers un centre du
                      sommeil.
                    </p>
                  </div>
                </div>
            ...
            <div className="text-xs text-gray-500 pt-2 border-t">
              <p>
                <strong>Epworth:</strong> Échelle de Somnolence d'Epworth
                (Epworth Sleepiness Scale)
              </p>
              <p className="mt-1">
                <strong>Cotation:</strong> 8 situations de la vie quotidienne
                (0-3 chacune). Score total: 0-24
              </p>
              <p className="mt-1">
                <strong>Situations:</strong> Lecture, TV, inactivité publique,
                passager voiture, repos après-midi, conversation, après repas,
                voiture arrêtée
              </p>
              <p className="mt-1">
                <strong>Seuils:</strong> 0-5 (normale basse) | 6-10 (normale
                haute) | 11-12 (légère) | 13-15 (modérée) | 16-24 (sévère)
              </p>
              <p className="mt-1 text-gray-600">
                <strong>Note:</strong> Score &gt; 10 indique une somnolence
                diurne excessive pathologique. Outil de dépistage des troubles
                du sommeil (SAOS, narcolepsie)
              </p>
            </div>
          </div>
        )}
        ...
        {code === "FAGERSTROM_SZ" && (
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <p className="text-[10px] text-gray-400 mt-1">
                  Q1 + Q4 (items les plus prédictifs)
                </p>
              </div>
            </div>
            ...
            <div className="text-[10px] text-gray-400 italic pt-2 text-right">
              FTND : Fagerström Test for Nicotine Dependence | HSI : Heaviness
              of Smoking Index
            </div>
          </div>
        )}
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-gray-500">
                    Organisation, adaptation, apprentissage, attention
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-gray-500">
                    Initiative, projet, utilisation du temps, curiosité
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-gray-500">
                    Empathie cognitive/émotionnelle, rôles sociaux
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <span className="text-gray-700 font-medium">
                      Capacité d'autoévaluation de ses capacités et de prise en
                  ...
                  <p className="text-[10px] text-gray-500">
                    Insight, demande d'aide, coopération aux soins
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-xs text-blue-800">
                <span className="font-semibold">Note clinique:</span> L'EPHP est
                un outil d'évaluation du handicap psychique rempli par
                l'entourage/aidant. Les scores élevés indiquent un meilleur
                fonctionnement. Cette échelle est utile pour les demandes
                PCH/AAH et le suivi en réhabilitation.
              </p>
            </div>
          ...
          <div className="text-sm space-y-4 mt-2 pt-2 border-t">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">
                    Bien-être psychologique (Q16-18):
                  </span>
                  <span
