...
import { saveSocialResponse } from "@/lib/services/questionnaire-social.service";
import {
  saveTobaccoResponse,
...
import {
  ...
  saveBisResponse as saveSchizophreniaBisResponse,
  saveEq5d5lSzResponse as saveSchizophreniaEq5d5lResponse,
  saveIpaqSzResponse as saveSchizophreniaIpaqResponse,
  saveYbocsResponse as saveSchizophreniaYbocsResponse,
  saveWurs25SzResponse as saveSchizophreniaWurs25Response,
  saveStoriSzResponse as saveSchizophreniaStoriResponse,
...
import {
  ...
  saveDepressionInclusionResponse,
...
async function shouldUseDepressionTables(
  ...
  const depressionCodes = ['QIDS_SR16', 'MADRS', 'THASE_RUSH', 'MINI', 'INCLUSION'];
...
async function shouldUseSchizophreniaTables(
  visitId: string,
  questionnaireCode: string,
): Promise<boolean> {
  // Check if the questionnaire code maps to a schizophrenia_* table
  const schizophreniaTableCode =
    questionnaireCodeToSchizophreniaKey(questionnaireCode);
  if (
    !schizophreniaTableCode ||
    !SCHIZOPHRENIA_INITIAL_TABLES[schizophreniaTableCode]
  ) {
    return false;
  }
...
function questionnaireCodeToSchizophreniaKey(code: string): string | null {
  const mapping: Record<string, string> = {
    ...
    COMPORTEMENTS_VIOLENTS_SZ: "COMPORTEMENTS_VIOLENTS_SZ",
    TROUBLES_COMORBIDES_SZ: "TROUBLES_COMORBIDES_SZ",
    TROUBLES_COMORBIDES_ANNUEL_SZ: "TROUBLES_COMORBIDES_ANNUEL_SZ",
    ISA: "ISA_SZ", // Map ISA to ISA_SZ for schizophrenia
    ISA_SZ: "ISA_SZ",
    ISA_SUIVI_SZ: "ISA_SUIVI_SZ",
    SUICIDE_HISTORY_SZ: "SUICIDE_HISTORY_SZ",
    SUICIDE_HISTORY_SUIVI_SZ: "SUICIDE_HISTORY_SUIVI_SZ",
...
export async function submitProfessionalQuestionnaireAction(
  ...
  try {
    ...
    if (useDepressionTables) {
      ...
      if (questionnaireCode === 'QIDS_SR16' || questionnaireCode === 'QIDS_SR16_FR') {
        ...
        result = await saveDepressionInclusionResponse({
          ...
          completed_by: completedBy,
          ...(responses as any),
        } as DepressionInclusionResponseInsert);
      } else {
    ...
    const useBipolarTables = await shouldUseBipolarTables(
      visitId,
      questionnaireCode,
    );
    ...
    if (useBipolarTables) {
      ...
      if (bipolarKey) {
        ...
        try {
          ...
          if (
            storedCompletion === 100 ||
            completionStatus.completionPercentage === 100
          ) {
            if (visit?.status !== "completed") {
              await completeVisit(visitId);
              console.log(
                `Visit ${visitId} automatically completed (100% questionnaires filled)`,
              );
            }
          }
        } catch (error) {
    ...
    const useSchizophreniaTables = await shouldUseSchizophreniaTables(
      visitId,
      questionnaireCode,
    );

    if (useSchizophreniaTables) {
      const schizophreniaKey =
        questionnaireCodeToSchizophreniaKey(questionnaireCode);
      if (schizophreniaKey) {
        // Filter out fields that are injected for form computation but may not be DB columns
        const {
    ...
    switch (questionnaireCode) {
      ...
      case "BILAN_SOCIAL_SZ":
        ...
        if (
          bilanSocialData.justice_safeguard !== undefined &&
          bilanSocialData.justice_safeguard !== null
        ) {
          bilanSocialData.justice_safeguard =
            bilanSocialData.justice_safeguard === 1;
        }
      ...
      case "COMMISSIONS_SZ":
        result = await saveSchizophreniaCommissionsSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "WAIS4_EFFICIENCE_SZ":
        result = await saveSchizophreniaWais4EfficienceSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "WAIS4_SIMILITUDES_SZ":
        result = await saveSchizophreniaWais4SimilitudesSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "WAIS4_MEMOIRE_CHIFFRES_SZ":
        console.log(
          "[Server Action] WAIS4_MEMOIRE_CHIFFRES_SZ - saving response",
        );
        ...
        console.log(
          "[Server Action] WAIS4_MEMOIRE_CHIFFRES_SZ - result:",
          result ? "success" : "null",
          "wais_mc_std:",
          result?.wais_mc_std,
        );
      ...
      case "WAIS4_MATRICES_SZ":
        result = await saveSchizophreniaWais4MatricesSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "SSTICS_SZ":
        result = await saveSchizophreniaSsticsSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "CBQ_SZ":
        result = await saveSchizophreniaCbqSzResponse({
          visit_id: visitId,
          patient_id: patientId,
          ...(responses as any),
        });
      ...
      case "CTI":
        throw new Error(
          `Questionnaire ${questionnaireCode} should be routed through shouldUseBipolarTables. Tables have been migrated to bipolar_* schema.`,
        );
      ...
      case "TOBACCO":
        console.log(
          "[submitProfessionalQuestionnaireAction] TOBACCO - responses:",
          JSON.stringify(responses, null, 2),
        );
        console.log(
          "[submitProfessionalQuestionnaireAction] TOBACCO - visitId:",
          visitId,
          "patientId:",
          patientId,
          "completedBy:",
        ...
        console.log(
          "[submitProfessionalQuestionnaireAction] TOBACCO - result:",
          result,
        );
        break;
    ...
    console.error(
      "[submitProfessionalQuestionnaireAction] Error stack:",
      error instanceof Error ? error.stack : "No stack trace",
    );
    console.error(
      "[submitProfessionalQuestionnaireAction] Error details:",
