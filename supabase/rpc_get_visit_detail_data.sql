-- =============================================================================
-- RPC: get_visit_detail_data
-- =============================================================================
-- FICHIER STABLE - Source unique de vérité pour la fonction get_visit_detail_data.
-- Ce fichier est exécuté automatiquement à chaque `supabase db reset` (via seed.sql).
--
-- WORKFLOW EQUIPE:
--   1. Pour ajouter un nouveau questionnaire au RPC, modifiez CE fichier uniquement.
--   2. NE CRÉEZ PAS de nouvelle migration timestampée pour modifier cette fonction.
--   3. Appliquez manuellement sur la DB distante si nécessaire:
--      psql -f supabase/rpc_get_visit_detail_data.sql
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    v_statuses := v_statuses || jsonb_build_object(
      ...
      'SANS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_sans WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_sans WHERE visit_id = p_visit_id)),
      'UKU', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_uku WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_uku WHERE visit_id = p_visit_id)),
    ...
    -- Schizophrenia medical module (initial) - renamed tables
    v_statuses := v_statuses || jsonb_build_object(
    ...
    v_statuses := v_statuses || jsonb_build_object(
      ...
      'CTQ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_ctq WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_ctq WHERE visit_id = p_visit_id)),
      ...
      'ALIMENTAIRE_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_alimentaire WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_alimentaire WHERE visit_id = p_visit_id)),
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      ...
      -- Medical module (ISA_SUIVI_SZ and SUICIDE_HISTORY_SUIVI_SZ replace their initial counterparts)
      v_statuses := v_statuses || jsonb_build_object(
    ...
    ELSE
      ...
      v_statuses := v_statuses || jsonb_build_object(
        ...
        'SIS_FOLLOWUP', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_followup_sis WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_followup_sis WHERE visit_id = p_visit_id)),
      ...
      -- Soin, suivi et arrêt de travail
      v_statuses := v_statuses || jsonb_build_object(
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status for all pathologies and visit types.';
