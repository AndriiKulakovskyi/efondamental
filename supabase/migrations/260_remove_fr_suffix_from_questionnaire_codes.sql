-- ============================================================================
-- Remove _FR Suffix from All Questionnaire Codes
-- ============================================================================
-- Update get_visit_detail_data RPC to return questionnaire statuses
-- with keys that match the actual questionnaire codes (without _FR suffix).
-- This standardizes naming across all questionnaires: screening, initial,
-- annual, biannual, and followup visits.
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- NOTE: Nurse questionnaire keys changed from *_FR to match actual questionnaire codes
  ...
  ELSIF v_visit_type = 'biannual_followup' THEN
    ...
    v_statuses := jsonb_build_object(
      ...
      'ETAT_PATIENT', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_etat_patient WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_etat_patient WHERE visit_id = p_visit_id)),
      'FAST', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_fast WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_fast WHERE visit_id = p_visit_id)),
      'HUMEUR_ACTUELS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_humeur_actuels WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_humeur_actuels WHERE visit_id = p_visit_id)),
      'HUMEUR_DEPUIS_VISITE', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_humeur_depuis_visite WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_humeur_depuis_visite WHERE visit_id = p_visit_id)),
      'PSYCHOTIQUES', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_psychotiques WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_psychotiques WHERE visit_id = p_visit_id)),
      'ISA_FOLLOWUP', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_isa_followup WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_isa_followup WHERE visit_id = p_visit_id)),
      'SUICIDE_BEHAVIOR_FOLLOWUP', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_suicide_behavior_followup WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_suicide_behavior_followup WHERE visit_id = p_visit_id)),
      'CSSRS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_cssrs WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_cssrs WHERE visit_id = p_visit_id)),
      'SUIVI_RECOMMANDATIONS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_suivi_recommandations WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_suivi_recommandations WHERE visit_id = p_visit_id)),
      'RECOURS_AUX_SOINS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_recours_aux_soins WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_recours_aux_soins WHERE visit_id = p_visit_id)),
      'TRAITEMENT_NON_PHARMACOLOGIQUE', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_traitement_non_pharmacologique WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_traitement_non_pharmacologique WHERE visit_id = p_visit_id)),
      'ARRETS_DE_TRAVAIL', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_arrets_de_travail WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_arrets_de_travail WHERE visit_id = p_visit_id)),
      'SOMATIQUE_CONTRACEPTIF', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_somatique_contraceptif WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_somatique_contraceptif WHERE visit_id = p_visit_id)),
      'STATUT_PROFESSIONNEL', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_statut_professionnel WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_statut_professionnel WHERE visit_id = p_visit_id))
    );
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed nurse questionnaire status keys in get_visit_detail_data RPC';
END $$;
