-- ============================================================================
-- eFondaMental Platform - Visit Detail RPC Function
-- ============================================================================
-- This migration creates an RPC function that aggregates all visit detail
-- data in a single database call, reducing 41 queries to just 1.
--
-- Performance improvement: 41 sequential queries -> 1 query
-- ============================================================================

-- Function to get all visit detail data with questionnaire statuses
...
BEGIN
  ...
  IF v_visit_type = 'screening' THEN
    -- Screening visit with 5 questionnaires
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- Initial evaluation visit with 40 questionnaires
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
    ...
    questionnaire_statuses AS (
      SELECT jsonb_build_object(
        ...
        -- DSM5 / Medical Eval questionnaires (7)
  ...
  ELSE
    -- Other visit types - return visit data only
    WITH visit_data AS (
      SELECT *
      FROM v_visits_full
      WHERE id = p_visit_id
...
'Aggregates all visit detail data with questionnaire completion statuses in a single database call. Performance: 41 sequential queries reduced to 1 query.';

-- Log completion
DO $$
BEGIN
  RAISE NOTICE 'Visit detail RPC function created successfully';
  RAISE NOTICE 'Performance optimization: 41 sequential queries reduced to 1 query';
END $$;
