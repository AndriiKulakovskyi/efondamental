-- ============================================================================
-- eFondaMental Platform - WAIS-IV Clinical Criteria Questionnaire Migration
-- ============================================================================
-- This migration creates the table for WAIS-IV Clinical Criteria
-- Part of the Neuropsychological Evaluation module for initial evaluation
-- ============================================================================

-- Create WAIS-IV Clinical Criteria table
CREATE TABLE responses_wais4_criteria (
...
CREATE INDEX idx_responses_wais4_criteria_visit ON responses_wais4_criteria(visit_id);
CREATE INDEX idx_responses_wais4_criteria_patient ON responses_wais4_criteria(patient_id);

-- ============================================================================
-- RLS Policies
-- ============================================================================

ALTER TABLE responses_wais4_criteria ENABLE ROW LEVEL SECURITY;

-- Patients can view their own responses
CREATE POLICY "Patients view own WAIS-4 criteria" 
ON responses_wais4_criteria FOR SELECT 
USING (auth.uid() = patient_id);

-- Professionals can view all responses
CREATE POLICY "Pros view all WAIS-4 criteria" 
ON responses_wais4_criteria FOR SELECT 
USING (
    EXISTS (
...
CREATE POLICY "Pros insert WAIS-4 criteria" 
ON responses_wais4_criteria FOR INSERT 
WITH CHECK (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE POLICY "Pros update WAIS-4 criteria" 
ON responses_wais4_criteria FOR UPDATE 
USING (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE TRIGGER update_responses_wais4_criteria_updated_at
    BEFORE UPDATE ON responses_wais4_criteria
    FOR EACH ROW
