-- ============================================================================
-- Migration: Add Schizophrenia Screening Questionnaires to RPC
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to support
-- pathology-specific screening questionnaires:
-- 
-- For Schizophrenia pathology:
--   - SCREENING_DIAGNOSTIC_SZ (responses_screening_sz_diagnostic)
--   - SCREENING_ORIENTATION_SZ (responses_screening_sz_orientation)
--
-- For other pathologies (Bipolar, etc.):
--   - ASRM_FR, QIDS_SR16_FR, MDQ_FR
--   - MEDICAL_DIAGNOSTIC_FR, BIPOLAR_ORIENTATION_FR
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'biannual_followup' THEN
    -- =========================================================================
    -- BIANNUAL FOLLOW-UP: 3 modules (unchanged)
    -- =========================================================================
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
      FROM visits v
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    -- =========================================================================
    -- ANNUAL EVALUATION: 5 modules (unchanged)
    -- =========================================================================
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
      FROM visits v
...
COMMENT ON FUNCTION get_visit_detail_data(UUID) IS 'Returns complete visit details including questionnaire completion status. Updated to support pathology-specific screening questionnaires (schizophrenia vs bipolar).';
