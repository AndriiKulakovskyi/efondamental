-- ============================================================================
-- Fix Biannual Followup Table Names in RPC
-- ============================================================================
-- This migration fixes incorrect table names in the get_visit_detail_data RPC
-- for biannual followup visits. The RPC was referencing tables without the
-- '_followup_' prefix, but the actual tables have this prefix.
--
-- Table name corrections:
--   bipolar_humeur_actuels          -> bipolar_followup_humeur_actuels
--   bipolar_humeur_depuis_visite    -> bipolar_followup_humeur_depuis_visite
--   bipolar_psychotiques            -> bipolar_followup_psychotiques
--   bipolar_isa_followup            -> bipolar_followup_isa
--   bipolar_suicide_behavior_followup -> bipolar_followup_suicide_behavior
--   bipolar_suivi_recommandations   -> bipolar_followup_suivi_recommandations
--   bipolar_recours_aux_soins       -> bipolar_followup_recours_aux_soins
--   bipolar_traitement_non_pharmacologique -> bipolar_followup_traitement_non_pharma
--   bipolar_arrets_de_travail       -> bipolar_followup_arrets_travail
--   bipolar_somatique_contraceptif  -> bipolar_followup_somatique_contraceptif
--   bipolar_statut_professionnel    -> bipolar_followup_statut_professionnel
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'biannual_followup' THEN
    ...
    -- FIXED: Using correct table names with _followup_ prefix
    v_statuses := jsonb_build_object(
      'TOBACCO', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_nurse_tobacco WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_nurse_tobacco WHERE visit_id = p_visit_id)),
      'FAGERSTROM', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_nurse_fagerstrom WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_nurse_fagerstrom WHERE visit_id = p_visit_id)),
      'PHYSICAL_PARAMS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM bipolar_nurse_physical_params WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM bipolar_nurse_physical_params WHERE visit_id = p_visit_id)),
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed biannual_followup table names in get_visit_detail_data RPC - now using bipolar_followup_* prefix';
END $$;
