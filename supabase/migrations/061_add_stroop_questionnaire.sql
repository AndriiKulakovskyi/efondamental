-- ============================================================================
-- eFondaMental Platform - Stroop Test (Golden 1978) Migration
-- ============================================================================
-- This migration creates the table for Stroop Test
-- Part of the Neuropsychological Evaluation module for initial evaluation
-- Evaluates executive functions and inhibition
-- ============================================================================

-- Create Stroop table
CREATE TABLE responses_stroop (
...
CREATE INDEX idx_responses_stroop_visit ON responses_stroop(visit_id);
CREATE INDEX idx_responses_stroop_patient ON responses_stroop(patient_id);

-- ============================================================================
-- RLS Policies
-- ============================================================================

ALTER TABLE responses_stroop ENABLE ROW LEVEL SECURITY;

-- Patients can view their own responses
CREATE POLICY "Patients view own Stroop" 
ON responses_stroop FOR SELECT 
USING (auth.uid() = patient_id);

-- Professionals can view all responses
CREATE POLICY "Pros view all Stroop" 
ON responses_stroop FOR SELECT 
USING (
    EXISTS (
...
CREATE POLICY "Pros insert Stroop" 
ON responses_stroop FOR INSERT 
WITH CHECK (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE POLICY "Pros update Stroop" 
ON responses_stroop FOR UPDATE 
USING (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE TRIGGER update_responses_stroop_updated_at
    BEFORE UPDATE ON responses_stroop
    FOR EACH ROW
