-- Migration: Add all schizophrenia questionnaires to get_visit_detail_data RPC function
-- This includes tracking for:
-- - Neuropsychological module (Bloc 2 + WAIS-IV)
-- - Social module
-- - Autoquestionnaires module (13 questionnaires)
-- - Autoquestionnaires entourage module

DROP FUNCTION IF EXISTS public.get_visit_detail_data(uuid);
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Schizophrenia hetero-questionnaires module
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Schizophrenia medical evaluation module
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Schizophrenia neuropsychological module (Bloc 2 + WAIS-IV)
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Schizophrenia autoquestionnaires module
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Schizophrenia autoquestionnaires entourage module
    v_statuses := v_statuses || jsonb_build_object(
...
-- Add comment
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status for bipolar and schizophrenia pathologies.';
