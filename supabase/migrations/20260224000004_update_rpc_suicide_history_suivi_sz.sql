-- Replace SUICIDE_HISTORY_SZ with SUICIDE_HISTORY_SUIVI_SZ in the annual_evaluation branch.
-- The initial_evaluation branch keeps SUICIDE_HISTORY_SZ -> schizophrenia_suicide_history unchanged.
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Schizophrenia medical module (initial)
    v_statuses := v_statuses || jsonb_build_object(
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      ...
      -- Medical module (TROUBLES_COMORBIDES_SZ removed; ISA_SUIVI_SZ and SUICIDE_HISTORY_SUIVI_SZ replace their initial counterparts)
      v_statuses := v_statuses || jsonb_build_object(
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. SUICIDE_HISTORY_SUIVI_SZ replaces SUICIDE_HISTORY_SZ in annual_evaluation for schizophrenia.';
