-- ============================================================================
-- Migration: Add EVAL_ADDICTOLOGIQUE_SZ to Schizophrenia Initial Evaluation RPC
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- the Evaluation Addictologique questionnaire completion tracking for 
-- schizophrenia initial evaluation visits (Addictologie section).
-- ============================================================================
...
BEGIN
  ...
  IF v_visit_type = 'screening' THEN
    -- SCREENING: Simple set of questionnaires
    WITH visit_data AS (
    ...
    SELECT jsonb_build_object(
      ...
      'questionnaire_statuses', jsonb_build_object(
        ...
        'CONSENT_SCREENING_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_screening_consent WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_screening_consent WHERE visit_id = p_visit_id)),
        'SOCIO_SCREENING_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_screening_socio WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_screening_socio WHERE visit_id = p_visit_id)),
        'DIAG_SCREENING_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_screening_diagnostic WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_screening_diagnostic WHERE visit_id = p_visit_id)),
        'ORIENTATION_SCREENING_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_screening_orientation WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_screening_orientation WHERE visit_id = p_visit_id)),
        'SZ_DIAGNOSTIC_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_sz_diagnostic WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_sz_diagnostic WHERE visit_id = p_visit_id)),
        'SZ_ORIENTATION_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_sz_orientation WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_sz_orientation WHERE visit_id = p_visit_id))
      ),
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Chunk 6: SAS + PSP + ECV + TROUBLES_PSYCHOTIQUES + TROUBLES_COMORBIDES_SZ + TEA_COFFEE_SZ + EVAL_ADDICTOLOGIQUE_SZ + schizophrenia suicide/family/perinatalite (10)
    v_statuses := v_statuses || jsonb_build_object(
      ...
      'TEA_COFFEE_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_tea_coffee WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_tea_coffee WHERE visit_id = p_visit_id)),
      'EVAL_ADDICTOLOGIQUE_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_eval_addictologique_sz WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_eval_addictologique_sz WHERE visit_id = p_visit_id)),
...
DO $$
BEGIN
  RAISE NOTICE 'Updated get_visit_detail_data RPC to include EVAL_ADDICTOLOGIQUE_SZ for schizophrenia initial evaluation';
END $$;
