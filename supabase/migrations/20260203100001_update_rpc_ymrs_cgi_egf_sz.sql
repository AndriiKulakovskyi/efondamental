-- Migration: Update get_visit_detail_data RPC to track YMRS_SZ, CGI_SZ, EGF_SZ completion
-- Purpose: Add new schizophrenia-specific YMRS, CGI, and EGF questionnaires to completion tracking
-- Date: 2026-02-03

-- Drop and recreate the function with updated schizophrenia hetero-questionnaires tracking
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Schizophrenia hetero module - UPDATED with YMRS_SZ, CGI_SZ, EGF_SZ
    v_statuses := v_statuses || jsonb_build_object(
      ...
      'BRIEF_A_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_brief_a WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_brief_a WHERE visit_id = p_visit_id)),
  ...
  ELSIF v_visit_type = 'annual_evaluation' OR v_visit_type = 'biannual_followup' OR v_visit_type = 'followup' THEN
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender, pa.type as pathology_type
      FROM visits v
      JOIN patients p ON v.patient_id = p.id
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Updated to include YMRS_SZ, CGI_SZ, and EGF_SZ for schizophrenia initial evaluation.';
