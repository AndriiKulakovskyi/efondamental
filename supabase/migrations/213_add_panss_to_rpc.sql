-- ============================================================================
-- Migration: Add PANSS to Schizophrenia Initial Evaluation RPC
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- the PANSS questionnaire completion tracking for schizophrenia initial 
-- evaluation visits.
--
-- Changes:
--   - Adds PANSS to schizophrenia initial evaluation questionnaire statuses
--   - Updates completion calculation to include PANSS
--   - Adds mod_hetero module tracking
-- ============================================================================
...
BEGIN
  ...
  IF v_visit_type = 'screening' THEN
    -- =========================================================================
    -- SCREENING VISIT: Pathology-specific questionnaires
    -- =========================================================================
    IF v_pathology_type = 'schizophrenia' THEN
      -- =====================================================================
      -- SCHIZOPHRENIA SCREENING: 2 questionnaires
      -- mod_medical: SCREENING_DIAGNOSTIC_SZ, SCREENING_ORIENTATION_SZ
      -- =====================================================================
      WITH visit_data AS (
      ...
      completion_calc AS (
        SELECT 2 AS total_questionnaires,
          ((CASE WHEN EXISTS (SELECT 1 FROM responses_screening_sz_diagnostic WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END) +
           (CASE WHEN EXISTS (SELECT 1 FROM responses_screening_sz_orientation WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END)) AS completed_questionnaires
      )
      SELECT jsonb_build_object(
    ...
    ELSE
      -- =====================================================================
      -- BIPOLAR/OTHER SCREENING: 5 questionnaires
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- =========================================================================
    -- INITIAL EVALUATION: Pathology-specific questionnaires
    -- =========================================================================
    IF v_pathology_type = 'schizophrenia' THEN
      -- =====================================================================
      -- SCHIZOPHRENIA INITIAL EVALUATION: 3 questionnaires
      -- mod_nurse: INF_DOSSIER_INFIRMIER (responses_dossier_infirmier_sz)
      --            INF_BILAN_BIOLOGIQUE_SZ (responses_bilan_biologique_sz)
      -- mod_hetero: PANSS (responses_panss)
      -- =====================================================================
      WITH visit_data AS (
        SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
        FROM visits v
      ...
      completion_calc AS (
        SELECT 3 AS total_questionnaires,
          ((CASE WHEN EXISTS (SELECT 1 FROM responses_dossier_infirmier_sz WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END) +
           (CASE WHEN EXISTS (SELECT 1 FROM responses_bilan_biologique_sz WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END) +
           (CASE WHEN EXISTS (SELECT 1 FROM responses_panss WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END)) AS completed_questionnaires
      )
      SELECT jsonb_build_object(
        'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
        'questionnaire_statuses', (SELECT statuses FROM questionnaire_statuses),
    ...
    ELSE
      -- =====================================================================
      -- BIPOLAR/OTHER INITIAL EVALUATION: All modules
      -- Split jsonb_build_object calls to avoid 100 argument limit
      -- =====================================================================
      WITH visit_data AS (
        SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
        FROM visits v
      ...
      -- Split questionnaire statuses into multiple parts to avoid 100 arg limit
      qs_nurse AS (
        SELECT jsonb_build_object(
      ...
      combined_statuses AS (
        SELECT (SELECT statuses FROM qs_nurse) || 
               (SELECT statuses FROM qs_thymic) || 
               (SELECT statuses FROM qs_medical_dsm5) || 
               (SELECT statuses FROM qs_suicide) || 
               (SELECT statuses FROM qs_somatic) || 
               (SELECT statuses FROM qs_neuropsy_root) || 
               (SELECT statuses FROM qs_wais3) || 
               (SELECT statuses FROM qs_wais4) || 
               (SELECT statuses FROM qs_auto_etat) || 
               (SELECT statuses FROM qs_social_traits) AS statuses
      )
      SELECT jsonb_build_object(
        'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
        'questionnaire_statuses', (SELECT statuses FROM combined_statuses),
  ...
  ELSIF v_visit_type = 'biannual_followup' THEN
    -- =========================================================================
    -- BIANNUAL FOLLOW-UP: 3 modules (bipolar only - schizophrenia doesn't have this)
    -- =========================================================================
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
      FROM visits v
    ...
    combined_statuses AS (
      SELECT (SELECT statuses FROM qs_nurse) || 
             (SELECT statuses FROM qs_medical) || 
             (SELECT statuses FROM qs_medical2) || 
             (SELECT statuses FROM qs_thymic) AS statuses
    )
    SELECT jsonb_build_object(
      'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
      'questionnaire_statuses', (SELECT statuses FROM combined_statuses),
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    -- =========================================================================
    -- ANNUAL EVALUATION: 5 modules (used by both pathologies)
    -- Split into multiple CTEs to avoid 100 argument limit
    -- =========================================================================
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
      FROM visits v
    ...
    combined_statuses AS (
      SELECT (SELECT statuses FROM qs_nurse) || 
             ...
             (SELECT statuses FROM qs_medical_dsm5) || 
             (SELECT statuses FROM qs_somatic) || 
             (SELECT statuses FROM qs_soin_suivi) || 
             (SELECT statuses FROM qs_neuropsy_root) || 
             (SELECT statuses FROM qs_wais3) || 
             (SELECT statuses FROM qs_wais4) || 
             (SELECT statuses FROM qs_auto) AS statuses
    )
    SELECT jsonb_build_object(
      'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
      'questionnaire_statuses', (SELECT statuses FROM combined_statuses),
...
COMMENT ON FUNCTION get_visit_detail_data(UUID) IS 'Returns complete visit details including questionnaire completion status. Updated to include PANSS in schizophrenia initial evaluation. Supports pathology-specific questionnaires for screening, initial evaluation, biannual follow-up, and annual evaluation visit types.';

-- Log completion
DO $$
BEGIN
  RAISE NOTICE 'Updated get_visit_detail_data RPC to include PANSS for schizophrenia initial evaluation';
END $$;
