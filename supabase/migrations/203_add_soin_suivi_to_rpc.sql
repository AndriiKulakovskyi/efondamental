-- ============================================================================
-- Migration: Add DSM5 Semestrial and Soin Suivi Questionnaires to RPC
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include:
-- 1. DSM5 semestrial questionnaire statuses for biannual_followup visits
-- 2. Soin, suivi et arrêt de travail questionnaire statuses for both
--    biannual_followup and annual_evaluation visits
--
-- New questionnaires added:
-- For biannual_followup DSM5 section:
--   - DIAG_PSY_SEM_HUMEUR_ACTUELS_FR
--   - DIAG_PSY_SEM_HUMEUR_DEPUIS_VISITE_FR
--   - DIAG_PSY_SEM_PSYCHOTIQUES_FR
-- For both biannual_followup and annual_evaluation (Soin, suivi section):
--   - SUIVI_RECOMMANDATIONS_FR
--   - RECOURS_AUX_SOINS_FR
--   - TRAITEMENT_NON_PHARMACOLOGIQUE_FR
--   - ARRETS_DE_TRAVAIL_FR
--   - SOMATIQUE_ET_CONTRACEPTIF_FR
--   - STATUT_PROFESSIONNEL_FR
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- =========================================================================
    -- INITIAL EVALUATION: All modules (unchanged)
    -- =========================================================================
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
      FROM visits v
  ...
  ELSIF v_visit_type = 'biannual_followup' THEN
    ...
    --   dsm5: 3 (DIAG_PSY_SEM_HUMEUR_ACTUELS, DIAG_PSY_SEM_HUMEUR_DEPUIS_VISITE, DIAG_PSY_SEM_PSYCHOTIQUES)
    --   suicide: 3 (ISA, SUICIDE_BEHAVIOR_FOLLOWUP, CSSRS)
    --   soin_suivi: 6 (SUIVI_RECOMMANDATIONS, RECOURS_AUX_SOINS, TRAITEMENT_NON_PHARMACOLOGIQUE, ARRETS_DE_TRAVAIL, SOMATIQUE_ET_CONTRACEPTIF, STATUT_PROFESSIONNEL)
    ...
    questionnaire_statuses AS (
      SELECT jsonb_build_object(
        ...
        'SLEEP_APNEA_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_sleep_apnea WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_sleep_apnea WHERE visit_id = p_visit_id)),
        'BIOLOGICAL_ASSESSMENT_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_biological_assessment WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_biological_assessment WHERE visit_id = p_visit_id)),
        -- mod_medical_eval > dsm5 section (3) - NEW: DSM5 semestrial questionnaires
        ...
        'CSSRS_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_cssrs WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_cssrs WHERE visit_id = p_visit_id)),
        -- mod_medical_eval > soin_suivi section (6) - NEW
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    -- =========================================================================
    -- ANNUAL EVALUATION: 5 modules (including mod_neuropsy)
    -- mod_nurse: 7 (TOBACCO, FAGERSTROM, PHYSICAL_PARAMS, BLOOD_PRESSURE, ECG, SLEEP_APNEA, BIOLOGICAL_ASSESSMENT)
    -- mod_thymic_eval: 7 (MADRS, ALDA, YMRS, FAST, CGI, EGF, ETAT_PATIENT)
    -- mod_medical_eval sections:
    --   dsm5: 5 (DSM5_HUMEUR, DSM5_PSYCHOTIC, DSM5_COMORBID, DIVA, FAMILY_HISTORY)
    ...
    --   soin_suivi: 6 (SUIVI_RECOMMANDATIONS, RECOURS_AUX_SOINS, TRAITEMENT_NON_PHARMACOLOGIQUE, ARRETS_DE_TRAVAIL, SOMATIQUE_ET_CONTRACEPTIF, STATUT_PROFESSIONNEL) - NEW
...
COMMENT ON FUNCTION get_visit_detail_data(UUID) IS 'Returns complete visit details including questionnaire completion status. Updated to include DSM5 semestrial and Soin, suivi et arrêt de travail questionnaires for biannual_followup and annual_evaluation visits.';
