-- ============================================================================
-- Migration: Sync get_visit_detail_data RPC keys with UI constants
-- Description: Align questionnaire status keys and add support for all visit types
-- ============================================================================

-- Refactor get_visit_detail_data to avoid 'cannot pass more than 100 arguments to a function' error.
-- We build the statuses object incrementally using the || operator.
...
BEGIN
  ...
  ELSIF v_visit_type IN ('initial_evaluation', 'biannual_followup', 'annual_evaluation', 'off_schedule') THEN
    ...
    -- 2. Schizophrenia specific statuses
    v_statuses := v_statuses || jsonb_build_object(
