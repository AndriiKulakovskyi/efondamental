-- Rename initial visit tables to make their scope explicit.
-- PostgreSQL RENAME preserves all constraints, indexes, triggers, RLS policies, and foreign keys.
ALTER TABLE public.schizophrenia_isa RENAME TO schizophrenia_isa_initial;
ALTER TABLE public.schizophrenia_suicide_history RENAME TO schizophrenia_suicide_history_initial;

-- Update the RPC to reference the new table names.
-- Only the initial_evaluation branch references these tables; the annual_evaluation
-- branch already uses schizophrenia_isa_suivi and schizophrenia_suicide_history_suivi.
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Tables renamed: schizophrenia_isa_initial (was schizophrenia_isa), schizophrenia_suicide_history_initial (was schizophrenia_suicide_history).';
