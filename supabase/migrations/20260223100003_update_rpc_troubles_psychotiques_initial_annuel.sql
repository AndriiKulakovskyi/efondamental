...
-- 1. initial_evaluation: TROUBLES_PSYCHOTIQUES -> TROUBLES_PSYCHOTIQUES_INITIAL (renamed table)
-- 2. annual_evaluation: TROUBLES_PSYCHOTIQUES -> TROUBLES_PSYCHOTIQUES_ANNUEL (new table)
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Schizophrenia medical module (CHANGED: TROUBLES_PSYCHOTIQUES -> TROUBLES_PSYCHOTIQUES_INITIAL)
    v_statuses := v_statuses || jsonb_build_object(
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      ...
      -- Medical module (CHANGED: TROUBLES_PSYCHOTIQUES -> TROUBLES_PSYCHOTIQUES_ANNUEL for annual visits)
      v_statuses := v_statuses || jsonb_build_object(
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Initial visits use TROUBLES_PSYCHOTIQUES_INITIAL; annual visits use TROUBLES_PSYCHOTIQUES_ANNUEL.';
