-- ============================================================================
-- eFondaMental Platform - WAIS-III CVLT Questionnaire Migration
-- ============================================================================
-- This migration creates the table for WAIS-III CVLT (California Verbal Learning Test)
-- Identical structure to WAIS-IV CVLT but stored separately
-- Part of the Neuropsychological Evaluation module
-- ============================================================================

-- Create WAIS-III CVLT table
CREATE TABLE responses_wais3_cvlt (
...
-- Enable RLS
ALTER TABLE responses_wais3_cvlt ENABLE ROW LEVEL SECURITY;

-- Professional Policies (SELECT, INSERT, UPDATE)
CREATE POLICY "Professionals can view WAIS3 CVLT responses"
ON responses_wais3_cvlt FOR SELECT
USING (
    EXISTS (
...
CREATE POLICY "Professionals can insert WAIS3 CVLT responses"
ON responses_wais3_cvlt FOR INSERT
WITH CHECK (
    EXISTS (
        SELECT 1 FROM user_profiles
        WHERE id = auth.uid()
...
CREATE POLICY "Professionals can update WAIS3 CVLT responses"
ON responses_wais3_cvlt FOR UPDATE
USING (
    EXISTS (
        SELECT 1 FROM user_profiles
        WHERE id = auth.uid()
...
-- Patient Policies
CREATE POLICY "Patients view own WAIS3 CVLT"
ON responses_wais3_cvlt FOR SELECT
USING (auth.uid() = patient_id);

-- Create indexes for performance
CREATE INDEX idx_responses_wais3_cvlt_visit_id ON responses_wais3_cvlt(visit_id);
CREATE INDEX idx_responses_wais3_cvlt_patient_id ON responses_wais3_cvlt(patient_id);

-- Add trigger for updated_at
CREATE TRIGGER update_responses_wais3_cvlt_updated_at
    BEFORE UPDATE ON responses_wais3_cvlt
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Add comment
COMMENT ON TABLE responses_wais3_cvlt IS 'Responses for WAIS-III California Verbal Learning Test (CVLT) - French Adaptation';
