-- ============================================================================
-- eFondaMental Platform - Update Visit Detail RPC for Stroop
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- Stroop Test questionnaire (48 total questionnaires)
-- ============================================================================

-- Drop existing function
DROP FUNCTION IF EXISTS get_visit_detail_data(UUID);

-- Recreate function with Stroop support
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- Initial evaluation visit with 48 questionnaires (47 + Stroop)
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
    ...
    questionnaire_statuses AS (
      SELECT jsonb_build_object(
        ...
        -- Neuropsychological Evaluation (8: WAIS-4 + CVLT + Code + Digit Span + TMT + Stroop)
    ...
    completion_calc AS (
      SELECT
        ...
        (
          ...
          -- Neuropsy: WAIS-4 + CVLT + Code + Digit Span + TMT + Stroop (8)
