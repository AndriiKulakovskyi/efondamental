-- ============================================================================
-- eFondaMental Platform - Update Visit Detail RPC for TMT
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- Trail Making Test (TMT) questionnaire (47 total questionnaires)
-- ============================================================================

-- Drop existing function
DROP FUNCTION IF EXISTS get_visit_detail_data(UUID);

-- Recreate function with TMT support
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- Initial evaluation visit with 47 questionnaires (46 + TMT)
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
    ...
    questionnaire_statuses AS (
      SELECT jsonb_build_object(
        ...
        -- Neuropsychological Evaluation (7: WAIS-4 Criteria + Learning + Matrices + CVLT + Code + Digit Span + TMT)
    ...
    completion_calc AS (
      SELECT
        ...
        (
          ...
          -- Neuropsy: WAIS-4 + CVLT + Code + Digit Span + TMT (7)
