-- ============================================================================
-- eFondaMental Platform - Update Visit Detail RPC for WAIS-III TMT
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- WAIS-III TMT questionnaire (56 total questionnaires)
-- ============================================================================

-- Drop existing function
DROP FUNCTION IF EXISTS get_visit_detail_data(UUID);

-- Recreate function with WAIS-III TMT support
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- Initial evaluation visit with 56 questionnaires
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
    ...
    -- HETERO + SOCIAL questionnaires (8)
    statuses_hetero AS (
      SELECT jsonb_build_object(
    ...
    -- DSM5 / Medical Eval questionnaires (9)
    statuses_dsm5 AS (
      SELECT jsonb_build_object(
    ...
    -- Neuropsychological Evaluation WAIS-IV (14)
    statuses_neuropsy_wais4 AS (
      SELECT jsonb_build_object(
        ...
        'WAIS4_CODE_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_wais4_code WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_wais4_code WHERE visit_id = p_visit_id)
        ),
        'WAIS4_DIGIT_SPAN_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_wais4_digit_span WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_wais4_digit_span WHERE visit_id = p_visit_id)
        ),
        'WAIS4_SIMILITUDES_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_wais4_similitudes WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_wais4_similitudes WHERE visit_id = p_visit_id)
        ),
        'TMT_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_tmt WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_tmt WHERE visit_id = p_visit_id)
        ),
        'STROOP_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_stroop WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_stroop WHERE visit_id = p_visit_id)
        ),
        'FLUENCES_VERBALES_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_fluences_verbales WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_fluences_verbales WHERE visit_id = p_visit_id)
        ),
        'COBRA_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_cobra WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_cobra WHERE visit_id = p_visit_id)
        ),
        'CPT3_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_cpt3 WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_cpt3 WHERE visit_id = p_visit_id)
        ),
        'TEST_COMMISSIONS_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_test_commissions WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_test_commissions WHERE visit_id = p_visit_id)
        ),
        'SCIP_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_scip WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_scip WHERE visit_id = p_visit_id)
        )
      ) AS statuses
    ),
    -- Neuropsychological Evaluation WAIS-III (2)
    statuses_neuropsy_wais3 AS (
      SELECT jsonb_build_object(
        'WAIS3_CVLT_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_wais3_cvlt WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_wais3_cvlt WHERE visit_id = p_visit_id)
        ),
        'WAIS3_TMT_FR', jsonb_build_object(
          'completed', EXISTS (SELECT 1 FROM responses_wais3_tmt WHERE visit_id = p_visit_id),
          'completed_at', (SELECT completed_at FROM responses_wais3_tmt WHERE visit_id = p_visit_id)
        )
      ) AS statuses
    ),
    -- Merge all status objects
    questionnaire_statuses AS (
      SELECT (
        (SELECT statuses FROM statuses_etat) ||
        (SELECT statuses FROM statuses_traits) ||
        (SELECT statuses FROM statuses_hetero) ||
        (SELECT statuses FROM statuses_infirmier) ||
        (SELECT statuses FROM statuses_dsm5) ||
        (SELECT statuses FROM statuses_neuropsy_wais4) ||
        (SELECT statuses FROM statuses_neuropsy_wais3)
      ) AS statuses
    ),
    completion_calc AS (
      SELECT
        56 AS total_questionnaires,
        (
          -- ETAT (9)
