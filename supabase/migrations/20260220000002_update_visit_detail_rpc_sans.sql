-- Migration: Update get_visit_detail_data RPC to include SANS questionnaire status
-- Adds SANS to both initial_evaluation (hetero module) and annual_evaluation (schizophrenia) branches

-- Add SANS status to the existing v_statuses in both branches
-- We append SANS via jsonb merge, just like other questionnaires are added
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Updated to include SANS for schizophrenia.';
