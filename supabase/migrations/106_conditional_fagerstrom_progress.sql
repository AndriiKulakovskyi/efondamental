-- ============================================================================
-- eFondaMental Platform - Conditional Fagerstrom Progress Calculation
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to:
-- 1. Check tobacco response smoking_status value
-- 2. Dynamically adjust total_questionnaires based on whether Fagerstrom is required
-- 3. Only count Fagerstrom as completed if it's required AND completed
--
-- Fagerstrom is only required when smoking_status = 'current_smoker' OR 'ex_smoker'
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    completion_calc AS (
      SELECT 
        -- Total is 66 if Fagerstrom required, 65 otherwise
        CASE WHEN v_fagerstrom_required THEN 66 ELSE 65 END AS total_questionnaires,
        (
          ...
          -- INFIRMIER questionnaires (5 or 6 depending on Fagerstrom)
