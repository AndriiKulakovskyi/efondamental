-- ============================================================================
-- eFondaMental Platform - Update Visit Detail RPC for WAIS-IV Digit Span
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to include
-- WAIS-IV Digit Span questionnaire (46 total questionnaires)
-- ============================================================================

-- Drop existing function
DROP FUNCTION IF EXISTS get_visit_detail_data(UUID);

-- Recreate function with WAIS-IV Digit Span support
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    -- Initial evaluation visit with 46 questionnaires (45 + 1 WAIS-4 Digit Span)
    WITH
    visit_data AS (
      SELECT *
      FROM v_visits_full
    ...
    questionnaire_statuses AS (
      SELECT jsonb_build_object(
        ...
        -- DSM5 / Medical Eval questionnaires (15: 9 DSM5 + 4 WAIS-4 + 1 CVLT + 1 WAIS-4 Code)
        ...
        -- Neuropsychological Evaluation (6: WAIS-4 Criteria + Learning + Matrices + CVLT + Code + Digit Span)
    ...
    completion_calc AS (
      SELECT
        ...
        (
          ...
          -- DSM5 + WAIS-4 + CVLT + Code + Digit Span (15)
