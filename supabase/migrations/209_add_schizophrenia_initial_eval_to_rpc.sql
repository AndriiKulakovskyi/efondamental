-- ============================================================================
-- Migration: Add Schizophrenia Initial Evaluation to RPC
-- ============================================================================
-- This migration updates the get_visit_detail_data RPC function to support
-- pathology-specific initial evaluation questionnaires:
-- 
-- For Schizophrenia pathology:
--   - INF_DOSSIER_INFIRMIER (responses_dossier_infirmier_sz)
--
-- For other pathologies (Bipolar, etc.):
--   - All existing bipolar questionnaires (unchanged)
--
-- Also ensures visit templates are properly deactivated for schizophrenia
-- ============================================================================

-- Ensure biannual_followup and off_schedule are deactivated for schizophrenia
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      -- =====================================================================
      -- SCHIZOPHRENIA INITIAL EVALUATION: 1 questionnaire
      ...
      completion_calc AS (
        SELECT 1 AS total_questionnaires,
          (CASE WHEN EXISTS (SELECT 1 FROM responses_dossier_infirmier_sz WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END) AS completed_questionnaires
      )
      SELECT jsonb_build_object(
        'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
        'questionnaire_statuses', (SELECT statuses FROM questionnaire_statuses),
...
COMMENT ON FUNCTION get_visit_detail_data(UUID) IS 'Returns complete visit details including questionnaire completion status. Updated to support pathology-specific initial evaluation questionnaires (schizophrenia vs bipolar). Refactored to use multiple CTEs to avoid PostgreSQL 100 argument limit for jsonb_build_object.';

-- Log completion
DO $$
BEGIN
  RAISE NOTICE 'Updated get_visit_detail_data RPC to support schizophrenia initial evaluation';
END $$;
