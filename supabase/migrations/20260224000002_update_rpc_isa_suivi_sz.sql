-- Replace ISA_SZ with ISA_SUIVI_SZ in the annual_evaluation branch of get_visit_detail_data.
-- The initial_evaluation branch keeps ISA_SZ -> schizophrenia_isa unchanged.
...
BEGIN
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      ...
      -- Medical module (TROUBLES_COMORBIDES_SZ removed from annual; ISA_SUIVI_SZ replaces ISA_SZ)
      v_statuses := v_statuses || jsonb_build_object(
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. ISA_SUIVI_SZ replaces ISA_SZ in annual_evaluation for schizophrenia.';
