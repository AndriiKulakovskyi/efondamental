-- ============================================================================
-- Migration: Update get_visit_detail_data RPC to include CVLT_SZ
-- Description: Add CVLT_SZ questionnaire status tracking for schizophrenia initial visits
-- ============================================================================

-- This migration updates the get_visit_detail_data function to include the
-- CVLT_SZ questionnaire status in the v_statuses JSON object.
...
BEGIN
  ...
  ELSIF v_visit_type IN ('initial_evaluation', 'biannual_followup', 'annual_evaluation', 'off_schedule') THEN
    ...
    -- 2. Schizophrenia specific statuses (including CVLT_SZ for Neuropsy Bloc 2)
    v_statuses := v_statuses || jsonb_build_object(
