-- ============================================================================
-- Migration: Add Bilan Biologique SZ to RPC
-- ============================================================================
-- Updates the get_visit_detail_data RPC function to include the new
-- INF_BILAN_BIOLOGIQUE_SZ questionnaire for schizophrenia initial evaluation
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      -- =====================================================================
      -- SCHIZOPHRENIA INITIAL EVALUATION: 2 questionnaires
      -- mod_nurse: INF_DOSSIER_INFIRMIER, INF_BILAN_BIOLOGIQUE_SZ
      -- =====================================================================
      WITH visit_data AS (
        SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender
        FROM visits v
      ...
      completion_calc AS (
        SELECT 2 AS total_questionnaires,
          ((CASE WHEN EXISTS (SELECT 1 FROM responses_dossier_infirmier_sz WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END) +
           (CASE WHEN EXISTS (SELECT 1 FROM responses_bilan_biologique_sz WHERE visit_id = p_visit_id) THEN 1 ELSE 0 END)) AS completed_questionnaires
      )
      SELECT jsonb_build_object(
        'visit', (SELECT row_to_json(vd.*) FROM visit_data vd),
        'questionnaire_statuses', (SELECT statuses FROM questionnaire_statuses),
...
COMMENT ON FUNCTION get_visit_detail_data(UUID) IS 'Returns complete visit details including questionnaire completion status. Updated to include INF_BILAN_BIOLOGIQUE_SZ questionnaire for schizophrenia initial evaluation.';

-- Log completion
DO $$
BEGIN
  RAISE NOTICE 'Updated get_visit_detail_data RPC to include Bilan Biologique SZ for schizophrenia initial evaluation';
END $$;
