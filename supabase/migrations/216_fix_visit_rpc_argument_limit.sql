-- ============================================================================
-- Migration: Fix get_visit_detail_data RPC - Split jsonb_build_object calls
-- ============================================================================
-- Issue: PostgreSQL's jsonb_build_object has a 100 argument limit (50 key-value pairs)
-- Solution: Split into multiple jsonb_build_object calls and merge with || operator
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Chunk 2: mod_medical_eval (19) = FAMILY_HISTORY + dsm5(4) + suicide(4) + histoire_somatique(10)
    ...
    -- Chunk 3: mod_neuropsy root (5) + wais3 (7) + wais4 part1 (5) = 17
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Chunk 4: wais4 part2 (5) + mod_auto_etat (9) = 14
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Chunk 5: mod_social (1) + mod_auto_traits (9) + schizophrenia (4) = 14
    v_statuses := v_statuses || jsonb_build_object(
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed get_visit_detail_data RPC to avoid 100 argument limit';
END $$;
