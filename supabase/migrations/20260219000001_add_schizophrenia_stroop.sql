-- Add Stroop Test (Golden 1978) table for schizophrenia vertical
-- Neuropsy Module - Bloc 2

-- ============================================================================
-- Table Creation
-- ============================================================================

CREATE TABLE IF NOT EXISTS "public"."schizophrenia_stroop" (
...
ALTER TABLE "public"."schizophrenia_stroop" OWNER TO "postgres";

COMMENT ON TABLE "public"."schizophrenia_stroop" IS 'Stroop Test (Golden 1978) responses for schizophrenia patients - Neuropsy Module Bloc 2. Measures selective attention and cognitive inhibition.';

-- ============================================================================
-- Primary Key and Unique Constraints
-- ============================================================================

ALTER TABLE ONLY "public"."schizophrenia_stroop"
    ADD CONSTRAINT "schizophrenia_stroop_pkey" PRIMARY KEY ("id");

ALTER TABLE ONLY "public"."schizophrenia_stroop"
    ADD CONSTRAINT "schizophrenia_stroop_visit_id_key" UNIQUE ("visit_id");
...
ALTER TABLE ONLY "public"."schizophrenia_stroop"
    ADD CONSTRAINT "schizophrenia_stroop_visit_id_fkey" FOREIGN KEY ("visit_id") REFERENCES "public"."visits"("id") ON DELETE CASCADE;

ALTER TABLE ONLY "public"."schizophrenia_stroop"
    ADD CONSTRAINT "schizophrenia_stroop_patient_id_fkey" FOREIGN KEY ("patient_id") REFERENCES "public"."patients"("id") ON DELETE CASCADE;

ALTER TABLE ONLY "public"."schizophrenia_stroop"
    ADD CONSTRAINT "schizophrenia_stroop_completed_by_fkey" FOREIGN KEY ("completed_by") REFERENCES "public"."user_profiles"("id");

-- ============================================================================
-- Indexes
-- ============================================================================

CREATE INDEX "idx_schizophrenia_stroop_patient_id" ON "public"."schizophrenia_stroop" USING "btree" ("patient_id");
CREATE INDEX "idx_schizophrenia_stroop_visit_id" ON "public"."schizophrenia_stroop" USING "btree" ("visit_id");

-- ============================================================================
-- Row Level Security
-- ============================================================================

ALTER TABLE "public"."schizophrenia_stroop" ENABLE ROW LEVEL SECURITY;

-- Patient policies
CREATE POLICY "Patients can view own schizophrenia_stroop" ON "public"."schizophrenia_stroop"
    FOR SELECT USING (("auth"."uid"() = "patient_id"));

CREATE POLICY "Patients can insert own schizophrenia_stroop" ON "public"."schizophrenia_stroop"
    FOR INSERT WITH CHECK (("auth"."uid"() = "patient_id"));

CREATE POLICY "Patients can update own schizophrenia_stroop" ON "public"."schizophrenia_stroop"
    FOR UPDATE USING (("auth"."uid"() = "patient_id"));

-- Professional policies
CREATE POLICY "Professionals can view schizophrenia_stroop" ON "public"."schizophrenia_stroop"
...
CREATE POLICY "Professionals can insert schizophrenia_stroop" ON "public"."schizophrenia_stroop"
    FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
       FROM "public"."user_profiles"
      WHERE (("user_profiles"."id" = "auth"."uid"()) AND ("user_profiles"."role" = ANY (ARRAY['healthcare_professional'::"public"."user_role", 'administrator'::"public"."user_role", 'manager'::"public"."user_role"]))))));

CREATE POLICY "Professionals can update schizophrenia_stroop" ON "public"."schizophrenia_stroop"
...
CREATE TRIGGER "handle_updated_at" BEFORE UPDATE ON "public"."schizophrenia_stroop"
    FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();

-- ============================================================================
-- Update RPC function to include STROOP_SZ in completion tracking
-- ============================================================================
...
BEGIN
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    IF v_pathology_type = 'schizophrenia' THEN
      -- Schizophrenia annual evaluation - track nurse and hetero questionnaires
      v_statuses := jsonb_build_object(
        'DOSSIER_INFIRMIER_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_dossier_infirmier WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_dossier_infirmier WHERE visit_id = p_visit_id)),
        'BILAN_BIOLOGIQUE_SZ', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_bilan_biologique WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_bilan_biologique WHERE visit_id = p_visit_id)),
        'PANSS', jsonb_build_object('completed', EXISTS (SELECT 1 FROM schizophrenia_panss WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM schizophrenia_panss WHERE visit_id = p_visit_id)),
    ...
    SELECT jsonb_build_object(
      ...
      'questionnaire_statuses', v_statuses,
      'completion_status', jsonb_build_object(
        'total_questionnaires', CASE WHEN v_pathology_type = 'schizophrenia' THEN 14 ELSE 0 END,
        'completed_questionnaires', 0,
        'completion_percentage', 0
      )
    ) INTO v_result;
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Updated to include STROOP_SZ for schizophrenia neuropsy module.';
