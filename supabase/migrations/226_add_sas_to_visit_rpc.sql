-- Update the get_visit_detail_data RPC function to include SAS completion status
-- This modifies the initial_evaluation chunk 5 to add SAS tracking
...
BEGIN
  ...
  IF v_visit_type = 'screening' THEN
    ...
    WITH visit_data AS (
      SELECT v.*, p.first_name, p.last_name, p.date_of_birth, p.gender, p.pathology_type
      FROM visits v
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Chunk 6: SAS (1) - separate chunk for new schizophrenia questionnaire
    v_statuses := v_statuses || jsonb_build_object(
