-- ============================================================================
-- Migration: Add schizophrenia_fluences_verbales table
-- Fluences Verbales (Cardebat et al., 1990) for Schizophrenia Initial Visit
-- ============================================================================

-- 1. Create the table
CREATE TABLE IF NOT EXISTS "public"."schizophrenia_fluences_verbales" (
...
ALTER TABLE "public"."schizophrenia_fluences_verbales" OWNER TO "postgres";

-- 2. Primary Key
ALTER TABLE "public"."schizophrenia_fluences_verbales"
    ADD CONSTRAINT "schizophrenia_fluences_verbales_pkey" PRIMARY KEY ("id");

-- 3. Unique constraint on visit_id (required for upsert)
ALTER TABLE "public"."schizophrenia_fluences_verbales"
    ADD CONSTRAINT "schizophrenia_fluences_verbales_visit_id_key" UNIQUE ("visit_id");

-- 4. Foreign keys
ALTER TABLE "public"."schizophrenia_fluences_verbales"
    ADD CONSTRAINT "schizophrenia_fluences_verbales_visit_id_fkey"
    FOREIGN KEY ("visit_id") REFERENCES "public"."visits"("id") ON DELETE CASCADE;

ALTER TABLE "public"."schizophrenia_fluences_verbales"
    ADD CONSTRAINT "schizophrenia_fluences_verbales_patient_id_fkey"
    FOREIGN KEY ("patient_id") REFERENCES "public"."patients"("id") ON DELETE CASCADE;

ALTER TABLE "public"."schizophrenia_fluences_verbales"
    ADD CONSTRAINT "schizophrenia_fluences_verbales_completed_by_fkey"
    FOREIGN KEY ("completed_by") REFERENCES "public"."user_profiles"("id");

-- 5. Indexes
CREATE INDEX IF NOT EXISTS "idx_sz_fluences_verbales_visit_id"
    ON "public"."schizophrenia_fluences_verbales" ("visit_id");

CREATE INDEX IF NOT EXISTS "idx_sz_fluences_verbales_patient_id"
    ON "public"."schizophrenia_fluences_verbales" ("patient_id");

-- 6. Updated_at trigger
CREATE TRIGGER "update_schizophrenia_fluences_verbales_updated_at"
    BEFORE UPDATE ON "public"."schizophrenia_fluences_verbales"
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

-- 7. Enable Row Level Security
ALTER TABLE "public"."schizophrenia_fluences_verbales" ENABLE ROW LEVEL SECURITY;

-- 8. RLS Policies - Patient
CREATE POLICY "Patients view own fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
    FOR SELECT USING (auth.uid() = patient_id);

CREATE POLICY "Patients insert own fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
    FOR INSERT WITH CHECK (auth.uid() = patient_id);

CREATE POLICY "Patients update own fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
    FOR UPDATE USING (auth.uid() = patient_id);

-- 9. RLS Policies - Professional
CREATE POLICY "Professionals view all fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
    FOR SELECT USING (
...
CREATE POLICY "Professionals insert fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
...
CREATE POLICY "Professionals update fluences_verbales_sz responses"
    ON "public"."schizophrenia_fluences_verbales"
    FOR UPDATE USING (
        EXISTS (SELECT 1 FROM user_profiles WHERE id = auth.uid()
        AND role IN ('healthcare_professional', 'manager', 'administrator'))
...
-- 10. Update RPC function to include FLUENCES_VERBALES_SZ in completion tracking
