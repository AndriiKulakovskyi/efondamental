-- Add SUICIDE_HISTORY_SZ and ANTECEDENTS_FAMILIAUX_PSY_SZ completion status to get_visit_detail_data RPC function
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Chunk 2: DSM5 + Suicide + Histoire somatique (14 items)
    ...
    -- Chunk 3: More histoire somatique + WAIS4 neuropsychology (14 items)
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Chunk 4: WAIS4 continued + auto questionnaires (14 items)
    v_statuses := v_statuses || jsonb_build_object(
      ...
      'WAIS3_CODE_FR', jsonb_build_object('completed', EXISTS (SELECT 1 FROM responses_wais3_code WHERE visit_id = p_visit_id), 'completed_at', (SELECT completed_at FROM responses_wais3_code WHERE visit_id = p_visit_id)),
    ...
    -- Chunk 5: Schizophrenia nurse + hetero-questionnaires (8 items)
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Chunk 6: SAS + PSP + ECV + TROUBLES_PSYCHOTIQUES + TROUBLES_COMORBIDES_SZ (5) - schizophrenia questionnaires
    v_statuses := v_statuses || jsonb_build_object(
    ...
    -- Chunk 7: SUICIDE_HISTORY_SZ + ANTECEDENTS_FAMILIAUX_PSY_SZ (2) - schizophrenia-specific questionnaires
    v_statuses := v_statuses || jsonb_build_object(
