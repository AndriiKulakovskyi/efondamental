...
CREATE TABLE public.schizophrenia_onaps (
    ...
    vpa_duration INTEGER,     -- Durée hebdomadaire d'activité physique de forte intensité (min/semaine)
    mpa_duration INTEGER,     -- Durée hebdomadaire d'activité physique d'intensité modérée (min/semaine)
    mvpa_duration INTEGER,    -- Durée hebdomadaire d'activité physique d'intensité modérée et de forte intensité (min/semaine)
    
    -- Computed scores (METs)
    vpamet INTEGER,           -- Activité physique de forte intensité en METs.min/semaine
    mpamet INTEGER,           -- Activité physique d'intensité modérée en METs.min/semaine
    aptot INTEGER,            -- Activité totale en METs.min/semaine
    
    -- Computed scores (Sédentarité)
    sb_duration INTEGER,      -- Comportement sédentaire (Sitting Behavior) total (min/jour)
    
    -- Interpretations
    mvpa_interpretation TEXT, -- INACTIF si < 150min/semaine, sinon ACTIF
    aptot_interpretation TEXT, -- InactiF si < 600 METs.min/semaine
    sedentarity_interpretation TEXT, -- Sédentarité faible si < 180 min/j
...
-- Professionals can view responses for their patients
...
-- Patients can update their own responses if not locked
...
-- 5. Update RPC for visit details
...
BEGIN
  ...
  ELSIF v_visit_type = 'initial_evaluation' THEN
    ...
    -- Schizophrenia neuropsychological module (Bloc 2 + WAIS-IV + CBQ + DACOBS + Fluences Verbales)
    v_statuses := v_statuses || jsonb_build_object(
  ...
  ELSIF v_visit_type = 'annual_evaluation' THEN
    ...
    SELECT jsonb_build_object(
      ...
      'questionnaire_statuses', v_statuses,
      'completion_status', jsonb_build_object(
        'total_questionnaires', CASE WHEN v_pathology_type = 'schizophrenia' THEN 15 ELSE 0 END,
        'completed_questionnaires', 0,
        'completion_percentage', 0
      )
    ) INTO v_result;
...
COMMENT ON FUNCTION "public"."get_visit_detail_data"("p_visit_id" "uuid") IS 'Returns complete visit details including questionnaire completion status. Updated to include ONAPS for schizophrenia.';
