-- ============================================================================
-- eFondaMental Platform - WAIS-III Trail Making Test (TMT) Migration
-- ============================================================================
-- This migration creates the table for WAIS-III Trail Making Test
-- Identical structure to WAIS-IV TMT but stored separately
-- Part of the Neuropsychological Evaluation module
-- ============================================================================

-- Create WAIS-III TMT table
CREATE TABLE responses_wais3_tmt (
...
CREATE INDEX idx_responses_wais3_tmt_visit ON responses_wais3_tmt(visit_id);
CREATE INDEX idx_responses_wais3_tmt_patient ON responses_wais3_tmt(patient_id);

-- ============================================================================
-- RLS Policies
-- ============================================================================

ALTER TABLE responses_wais3_tmt ENABLE ROW LEVEL SECURITY;

-- Patients can view their own responses
CREATE POLICY "Patients view own WAIS3 TMT" 
ON responses_wais3_tmt FOR SELECT 
USING (auth.uid() = patient_id);

-- Professionals can view all responses
CREATE POLICY "Pros view all WAIS3 TMT" 
ON responses_wais3_tmt FOR SELECT 
USING (
    EXISTS (
...
CREATE POLICY "Pros insert WAIS3 TMT" 
ON responses_wais3_tmt FOR INSERT 
WITH CHECK (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE POLICY "Pros update WAIS3 TMT" 
ON responses_wais3_tmt FOR UPDATE 
USING (
    EXISTS (
        SELECT 1 FROM user_profiles 
        WHERE id = auth.uid() 
...
CREATE TRIGGER update_responses_wais3_tmt_updated_at
    BEFORE UPDATE ON responses_wais3_tmt
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

-- Add comment
COMMENT ON TABLE responses_wais3_tmt IS 'Responses for WAIS-III Trail Making Test (TMT)';
