-- ============================================================================
-- Migration: Fix Screening Visit RPC Table References
-- ============================================================================
-- This migration fixes the get_visit_detail_data RPC function to use correct
-- table names for screening visits:
-- - Removes references to non-existent tables (responses_screening_consent, responses_screening_socio)
-- - Fixes schizophrenia table names: responses_sz_* -> responses_screening_sz_*
-- - Fixes bipolar table names: responses_screening_* -> responses_bipolar_*
-- - Updates status keys to match frontend expectations
-- ============================================================================
...
BEGIN
  ...
  IF v_visit_type = 'screening' THEN
    ...
    -- - Schizophrenia: responses_screening_sz_diagnostic, responses_screening_sz_orientation
    WITH visit_data AS (
    ...
    SELECT jsonb_build_object(
      ...
      'questionnaire_statuses', jsonb_build_object(
        ...
        -- Bipolar medical questionnaires (using correct table names)
        ...
        -- Schizophrenia medical questionnaires (using correct table names)
...
DO $$
BEGIN
  RAISE NOTICE 'Fixed get_visit_detail_data RPC to use correct table names for screening visits';
END $$;
