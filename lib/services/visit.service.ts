...
export async function getBulkVisitCompletionStatus(visitIds: string[]): Promise<Map<string, { completionPercentage: number; completedQuestionnaires: number; totalQuestionnaires: number }>> {
  ...
  if (initialEvalVisits.length > 0) {
    ...
    const [
      ...
      supabase.from('bipolar_tobacco').select('visit_id').in('visit_id', evalIds),
      supabase.from('bipolar_fagerstrom').select('visit_id').in('visit_id', evalIds),
      supabase.from('bipolar_physical_params').select('visit_id').in('visit_id', evalIds),
      supabase.from('bipolar_blood_pressure').select('visit_id').in('visit_id', evalIds),
      supabase.from('bipolar_sleep_apnea').select('visit_id').in('visit_id', evalIds),
      supabase.from('bipolar_biological_assessment').select('visit_id').in('visit_id', evalIds),
