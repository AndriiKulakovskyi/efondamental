// eFondaMental Platform - Biological Assessment (Bilan biologique)
...
export interface BipolarNurseBiologicalAssessmentResponse {
  id: string;
  visit_id: string;
  patient_id: string;
  // Date and Location
  ...
  // Psychotropes
  treatment_taken_morning: string | null;
  ...
  vitamin_d_supplementation: string | null;
  ...
  // Toxoplasmose
  toxo_serology_done: string | null;
  toxo_igm_positive: string | null;
  toxo_igg_positive: string | null;
  toxo_igg_value: number | null;
  toxo_pcr_done: string | null;
  toxo_pcr_positive: string | null;
  // Metadata
...
export type BipolarNurseBiologicalAssessmentResponseInsert = {
  visit_id: string;
  patient_id: string;
  // Date and Location
  collection_date?: string | null;
...
export const BIOLOGICAL_ASSESSMENT_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    help: 'Valeur normale comprise entre 80 et 120. Calculee par la formule de Cockroft et Gault.',
    type: 'number',
    required: false,
    min: 0,
    max: 10000
  ...
  {
    id: 'diabetes_warning',
    text: '***En cas de diabete (Glycemie > 7 mmol/L, soit > 1,26 g/L)***',
    type: 'text',
    required: false,
  ...
  {
    id: 'section_hcg',
    text: 'HCG - Femmes en age de procreer',
    type: 'section',
    required: false,
    display_if: {
  ...
  },
  {
    ...
    text: `
### Interpretation
| IgM | IgG | Interpretation possible |
| :--- | :--- | :--- |
| Negatif | Positif | Infection passee |
| Negatif | Negatif | Pas d'infection ou infection tres precoce |
| Positif | Negatif | Infection precoce |
...
export const BIOLOGICAL_ASSESSMENT_DEFINITION = {
...
export interface BiologicalAssessmentFlags {
  has_diabetes_risk: boolean;
  has_thyroid_abnormality: boolean;
  has_liver_abnormality: boolean;
  has_renal_abnormality: boolean;
  has_lipid_abnormality: boolean;
  has_lithium_toxicity_risk: boolean;
}

export interface BiologicalAssessmentInput {
  glycemie_a_jeun: number | null;
  glycemie_a_jeun_unit: string | null;
  tsh: number | null;
  asat: number | null;
...
export function analyzeBiologicalAssessment(input: BiologicalAssessmentInput): BiologicalAssessmentFlags {
  // Diabetes risk check
  let has_diabetes_risk = false;
  if (input.glycemie_a_jeun !== null) {
    if (input.glycemie_a_jeun_unit === 'mmol_L' && input.glycemie_a_jeun > 7) {
      has_diabetes_risk = true;
    } else if (input.glycemie_a_jeun_unit === 'g_L' && input.glycemie_a_jeun > 1.26) {
      has_diabetes_risk = true;
    }
  }

  // Thyroid abnormality check (TSH out of range 0.3-4.4)
  const has_thyroid_abnormality = input.tsh !== null && (input.tsh < 0.3 || input.tsh > 4.4);

  // Liver abnormality check (elevated transaminases)
  const has_liver_abnormality = (
    (input.asat !== null && input.asat > 40) ||
    (input.alat !== null && input.alat > 45) ||
    (input.ggt !== null && input.ggt > 65)
  );

  // Renal abnormality check (elevated creatinine)
  const has_renal_abnormality = input.creatinine !== null && input.creatinine > 110;

  // Lipid abnormality check
  const has_lipid_abnormality = (
    (input.cholesterol_total !== null && input.cholesterol_total > 6.1) ||
    (input.triglycerides !== null && input.triglycerides > 1.4)
  );

  // Lithium toxicity risk (level > 1.2)
  const has_lithium_toxicity_risk = input.lithium_plasma !== null && input.lithium_plasma > 1.2;

  return {
    has_diabetes_risk,
    has_thyroid_abnormality,
    has_liver_abnormality,
    has_renal_abnormality,
    has_lipid_abnormality,
    has_lithium_toxicity_risk
  };
}

export function interpretBiologicalAssessment(input: BiologicalAssessmentInput): string {
  const flags = analyzeBiologicalAssessment(input);
  const alerts: string[] = [];

  if (flags.has_diabetes_risk) alerts.push('Risque diabetique');
  if (flags.has_thyroid_abnormality) alerts.push('Anomalie thyroidienne');
  if (flags.has_liver_abnormality) alerts.push('Anomalie hepatique');
  if (flags.has_renal_abnormality) alerts.push('Anomalie renale');
  if (flags.has_lipid_abnormality) alerts.push('Anomalie lipidique');
  if (flags.has_lithium_toxicity_risk) alerts.push('Risque de toxicite au lithium');

  if (alerts.length === 0) {
    return 'Bilan biologique dans les limites normales.';
  }
  return `Alertes: ${alerts.join(', ')}.`;
}
