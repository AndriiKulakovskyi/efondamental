// eFondaMental Platform - WAIS-IV Digit Span (Memoire des Chiffres)
...
export interface BipolarWais4DigitSpanResponse {
  id: string;
  visit_id: string;
  patient_id: string;
  ...
  // Sequencing (Ordre croissant)
  sequencing_trial1_1: number | null;
  sequencing_trial1_2: number | null;
  sequencing_trial2_1: number | null;
  sequencing_trial2_2: number | null;
  sequencing_trial3_1: number | null;
  sequencing_trial3_2: number | null;
  sequencing_trial4_1: number | null;
  sequencing_trial4_2: number | null;
  sequencing_trial5_1: number | null;
  sequencing_trial5_2: number | null;
  sequencing_trial6_1: number | null;
  sequencing_trial6_2: number | null;
  sequencing_trial7_1: number | null;
  sequencing_trial7_2: number | null;
  sequencing_trial8_1: number | null;
  sequencing_trial8_2: number | null;
  sequencing_raw_score: number | null;
  sequencing_span: number | null;
  // Total scores
...
export type BipolarWais4DigitSpanResponseInsert = Omit<
  BipolarWais4DigitSpanResponse,
...
function createDigitSpanTrials(prefix: string, sectionTitle: string, maxLevel: number): Question[] {
  ...
  for (let level = 1; level <= maxLevel; level++) {
    const digits = level + 1; // Level 1 = 2 digits, Level 2 = 3 digits, etc.
    questions.push({
...
export const WAIS4_DIGIT_SPAN_QUESTIONS: Question[] = [
  {
    ...
    text: 'WAIS-IV Memoire des Chiffres',
    help: 'Trois conditions: Ordre Direct, Ordre Inverse, et Ordre Croissant.',
    type: 'section',
    required: false
  },
  ...createDigitSpanTrials('forward', 'Ordre Direct', 8),
  ...createDigitSpanTrials('backward', 'Ordre Inverse', 7),
  ...createDigitSpanTrials('sequencing', 'Ordre Croissant', 8)
];
...
export function computeWais4DigitSpanRawScore(
  forwardRaw: number,
  backwardRaw: number,
  sequencingRaw: number
): number {
  return forwardRaw + backwardRaw + sequencingRaw;
}
...
export const WAIS4_DIGIT_SPAN_DEFINITION: QuestionnaireDefinition = {
  ...
  title: 'WAIS-IV Memoire des Chiffres',
  description: 'Sous-test Memoire des Chiffres de la WAIS-IV - Evaluation de la memoire de travail auditive.',
