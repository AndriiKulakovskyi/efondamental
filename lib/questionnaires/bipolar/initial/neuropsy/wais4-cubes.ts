// eFondaMental Platform - WAIS-IV Cubes (Block Design)
...
export interface BipolarWais4CubesResponse {
  id: string;
  visit_id: string;
  patient_id: string;
  // Items 1-14
...
export type BipolarWais4CubesResponseInsert = Omit<
  BipolarWais4CubesResponse,
...
export const WAIS4_CUBES_QUESTIONS: Question[] = [
  {
...
export function computeWais4CubesRawScore(responses: Record<string, number>): number {
  let total = 0;
  for (let i = 1; i <= 14; i++) {
...
export const WAIS4_CUBES_DEFINITION: QuestionnaireDefinition = {
  id: 'wais4_cubes',
  code: 'WAIS4_CUBES',
  title: 'WAIS-IV Cubes',
  description: 'Sous-test Cubes de la WAIS-IV - Evaluation des capacites visuo-constructives et du raisonnement perceptif.',
  questions: WAIS4_CUBES_QUESTIONS,
  metadata: {
    singleColumn: true,
    pathologies: ['bipolar'],
