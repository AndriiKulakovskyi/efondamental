...
// Depression Screening - Auto-questionnaire
...
export interface DepressionQidsResponse {
  id: string;
  visit_id: string;
  patient_id: string;
  q1: number;
...
export type DepressionQidsResponseInsert = Omit<
  DepressionQidsResponse,
...
export const DEPRESSION_QIDS_QUESTIONS: Question[] = [
  {
    id: 'q1',
    text: 'Endormissement',
    type: 'single_choice',
...
export const DEPRESSION_QIDS_DEFINITION: QuestionnaireDefinition = {
  id: 'depression_qids_sr16',
  code: 'QIDS_SR16',
  title: 'QIDS-SR16',
  description: 'Auto-questionnaire court sur les symptomes de la depression - Periode de reference: 7 derniers jours',
  questions: DEPRESSION_QIDS_QUESTIONS,
  metadata: {
    singleColumn: true,
...
export interface DepressionQidsScoreInput {
  q1: number;
  q2: number;
  q3: number;
  q4: number;
...
/**
 * QIDS-SR16 domain-specific max scoring:
 * - Sleep (q1-q4): MAX
 * - Appetite/Weight (q6-q9): MAX
 * - Psychomotor (q15-q16): MAX
 * - Other items (q5, q10-q14): direct
 * Range: 0-27
 */
export function computeDepressionQidsScore(responses: DepressionQidsScoreInput): number {
...
export type DepressionQidsSeverity =
  | 'none'
  | 'mild'
  | 'moderate'
  | 'severe'
...
export interface DepressionQidsInterpretationResult {
  severity: DepressionQidsSeverity;
  label: string;
}

export function interpretDepressionQidsScore(score: number): string {
  if (score <= 5) {
...
export function getDepressionQidsSeverity(score: number): DepressionQidsInterpretationResult {
...
export interface DepressionQidsScoringResult {
  total_score: number;
...
export function scoreDepressionQids(responses: DepressionQidsScoreInput): DepressionQidsScoringResult {
  const sleepScore = Math.max(responses.q1, responses.q2, responses.q3, responses.q4);
  const appetiteWeightScore = Math.max(responses.q6, responses.q7, responses.q8, responses.q9);
  const psychomotorScore = Math.max(responses.q15, responses.q16);

  const total_score = computeDepressionQidsScore(responses);
  const interpretation = interpretDepressionQidsScore(total_score);
  const { severity } = getDepressionQidsSeverity(total_score);
